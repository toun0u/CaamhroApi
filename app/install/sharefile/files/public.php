	...
	case 'sharefile':
		switch($action) {
			...
			case 'admin_params':
				...
				include_once("./common/modules/sharefile/public_param.php");
				break;
	...
	case 'share':
		switch($action) {
			...
			case 'second_step':
				...
				if($share->hasEntity()) {
					include_once './common/modules/system/class_tiers.php';
					$entity = new tiers();
					$entity->open($share->getEntityId());

					$contactFields = array();

					include_once './common/modules/system/class_search.php';

					$search = new search();
					$result = $search-> exploreMatrice($_SESSION['dims']['workspaceid'],null,null,null,array($entity->fields['id_globalobject']));

					$params = array();
					$sql = 'SELECT * FROM '.contact::TABLE_NAME.' WHERE id_globalobject IN ('.$db->getParamsFromArray(array_keys($result['distribution']['contacts']), 'contacts', $params).')';
...
*/
			...
			case 'save_share':
				...
				$extraparams = '';
				if($share->hasEntity()) {
					include_once './common/modules/wce/include/classes/class_wce_site.php';
					include_once './common/modules/system/class_module_type.php';
					...
					$url = new dims_urlBuilder('index.php', '/', $baseUrl, '80', '');
				...
				if (dims_isadmin() || $share->isOwner($_SESSION['dims']['userid'])) {
					...
					if ($shareuser->fields['id_user']>0) {
						...
						$res=$db->query("SELECT DISTINCT u.id, u.email, u.firstname,u. lastname,fu.code
										...
										WHERE u.id = :userid",
										$arrayName = array('idshare' => $share->fields['id'], 'userid' => $shareuser->fields['id_user'])
										);
						if ($db->numrows($res)>0) {
						...
						//elseif (!isset($_SESSION['dims']['userid']) || (isset($_SESSION['dims']['userid']) && !$share->isOwner($_SESSION['dims']['userid']))) {
							...
							if (!isset($_SESSION['sharecount'][$id_share]) && !$share->isOwner($_SESSION['dims']['userid'])) {
								...
								if ($_SESSION['currentshare']['id_user']>0)
									...
									AND id_user= :userid",
									array(':idshare' => $share->fields['id'],':userid' => $_SESSION['currentshare']['id_user'])
									);
								else
									$res=$db->query("SELECT *
									FROM dims_mod_sharefile_user

	...
	<div id="shareblock1">
					...
					<?php foreach($entityContacts as $contact) : ?>
						<li>
							...
							<a class="action" href="Javascript: void(0);" onclick="Javascript: addContact('<?= $contact['id']; ?>');" >
								<img style="float:right" src="/modules/sharefile/img/icon_ajouter.png" />
							</a>
						</li>
					<?php endforeach ; ?>
					</ul>
		...
		<div style="float:left;width:100%;display:block;">
			<span style="display: block; float: left; width: 20%;">
				<img src="/modules/sharefile/img/gestion_contact.png">
			</span>
			...
			<div style="float: left; clear: left; width: 40%; line-height: 27px;">
				<a class="action" href="Javascript: void(0);" onclick="Javascript: $('#lst_tempuser, #add_contact').toggle();">
					...
					<img src="/modules/sharefile/img/icon_ajouter.png" style="float:right;"/>
				</a>
			</div>
			<div style="float: left; clear: left; width: 100%; margin-top: 20px;">
				...
				<img style="cursor: pointer; float: right; margin-right: 4px;" onclick="javascript:searchFileInitSearch();" src="/modules/sharefile/img/icon_mini_supp.png" border="0">
				<img style="cursor: pointer; float: right; margin-right: 10px;" onclick="" src="/modules/sharefile/img/icon_mini_loupe.png" border="0">
			</div>
			<div id="lst_tempuser" style="width:100%;display:block;float:left;"></div>

			<form action="<?= dims_urlencode(dims::getInstance()->getScriptEnv().'?op=share&action=save_add_contact'); ?>" id="add_contact" style="display:none;float:left;margin-top:30px;width:86%;">
						...
						$('#lst_user').append(
							'<li id="tmpuser'+data.users[user_id].id+'">'+
								'<div style="float: left; width: 100%;line-height:32px">'+
									'<img src="/modules/sharefile/img/icon_avatar.png" />'+ data.users[user_id].firstname+' '+data.users[user_id].lastname+
									'<a class="action" href="Javascript: void(0);" onclick="Javascript: removeUser('+data.users[user_id].id+');" >'+
										'<img style="float: right; padding-top: 5px;"src="/modules/sharefile/img/icon_mini_supp.png" />'+
									'</a>'+
								'</div>'+
							'</li>'
						);
				...
				if(data.contacts.length != 0) {
					...
					for(var contact_id in data.contacts) {
						$('#lst_contact').append(
							'<li id="tmpcontact'+data.contacts[contact_id].id+'">'+
								'<div style="float: left; width: 100%;line-height:32px">'+
									'<img src="/modules/sharefile/img/icon_avatar.png" />'+ data.contacts[contact_id].firstname+' '+data.contacts[contact_id].lastname+

...
*/
		...
		$zip_file=$zip_path."/".$this->fields['id']."_".$this->fields['timestp_create'].".zip";
			$zip_file_web="./data/sharefiles/".$this->fields['id']."_".$this->fields['timestp_create'].".zip";
	...
	public function getUsers() {
		...
		if(!empty($this->users)) {
			...
			$sqlUsers.= '
				WHERE		user.id IN ('.$db->getParamsFromArray($this->users, 'users', $params).')';
	...
	public function getContacts() {
		...
		if(!empty($this->contacts)) {
			...
			$sqlContact.= '
				WHERE		contact.id IN ('.$db->getParamsFromArray($this->contacts, 'contacts', $params).')';
	...
	public function getFiles() {
		...
		if(!empty($this->files)) {
			...
			$sqlFile = 'SELECT *
						...
						WHERE file.id IN  ('.$db->getParamsFromArray($this->files, 'files', $params).')';

			$res = $this->db->query($sqlFile, $params);
	...
	public function save() {
		...
		if($this->hasEntity()) {
			...
			include_once './common/modules/system/class_matrix.php';
			$params = array();
			$sql = 'SELECT id_globalobject FROM '.docfile::TABLE_NAME.' WHERE id IN ('.$db->getParamsFromArray($this->files, 'files', $params).')';

			$resFile = $this->db->query($sql, $params);

			$params = array();
			$sql = 'SELECT id_globalobject FROM '.contact::TABLE_NAME.' WHERE id IN ('.$db->getParamsFromArray($this->contacts, 'contacts', $params).')';

			$resContact = $this->db->query($sql, $params);

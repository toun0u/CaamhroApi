...
class NewsController extends DynObject{
			...
			switch($this->getParam('mode')){
				...
				case 'home':
					...
					if(!is_null($path) && file_exists(_WCE_MODELS_PATH."/objects/".$path)){
						...
						$this->setTPLPath(DIMS_APP_PATH.'templates/objects/news/index_home.tpl');
					}
					$dims = dims::getInstance();
					if(isset($_SESSION['dims']['front_template_name']) && $_SESSION['dims']['front_template_name'] != '' && file_exists(DIMS_WEB_PATH."/common/templates/frontoffice/".$_SESSION['dims']['front_template_name']."/styles_home.css")){
						$this->smarty->assign('styles_path', $dims->getProtocol().$dims->getHttpHost().'/common/templates/frontoffice/'.$_SESSION['dims']['front_template_name'].'/styles_home.css');
					} else {
						$this->smarty->assign('styles_path', $dims->getProtocol().$dims->getHttpHost().'/common/templates/objects/news/styles_home.css');
					}
					break;
				case 'rss':
					$this->buildRSS();
					$this->setTPLPath(DIMS_APP_PATH.'templates/objects/news/rss.tpl');
					break;
				case 'full_index':
					$this->buildView(true);
					$path = $this->getParam('path');
					if(!is_null($path) && file_exists(_WCE_MODELS_PATH."/objects/".$path)){
						...
						$this->setTPLPath(DIMS_APP_PATH.'templates/objects/news/full_index.tpl');
					}
					$dims = dims::getInstance();
					if(isset($_SESSION['dims']['front_template_name']) && $_SESSION['dims']['front_template_name'] != '' && file_exists(DIMS_WEB_PATH."/common/templates/frontoffice/".$_SESSION['dims']['front_template_name']."/full_styles.css")){
						$this->smarty->assign('styles_path', $dims->getProtocol().$dims->getHttpHost().'/common/templates/frontoffice/'.$_SESSION['dims']['front_template_name'].'/full_styles.css');
					} else {
						$this->smarty->assign('styles_path', $dims->getProtocol().$dims->getHttpHost().'/common/templates/objects/news/full_styles.css');
					}
					break;
			}
	...
	public function buildView($full = false){
		...
		if( is_null($max_elems) && !$full) $max_elems = 3;

		if( ! empty($max_elems) ) $limit = ' LIMIT 0, '.$max_elems;
		else $limit = '';
	...
	public function buildRSS(){
		...
		$res = $db->query("SELECT a.* FROM ".wce_article::TABLE_NAME." a
						   ...
						   ".($this->object->fields['pubfin_dependant'] ? " AND (".$now." <= a.timestp_unpublished OR a.timestp_unpublished=0) " : "")."
						   AND o.type = :type
						   AND o.id_module = :idm
						   GROUP BY a.id
						   ORDER BY a.timestp_published DESC
						   LIMIT 20", array(
				':type' => WCE_OBJECT_TYPE_NEWS,

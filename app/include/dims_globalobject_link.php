        ...
        if(!empty($tab_globalobject)){
            if($getting_type === dims_globalobject_link::$GET_TO_FROM){
                $sql_globalobject_link = "SELECT *
                    ...
                    WHERE   id_globalobject_from IN (".$db->getParamsFromArray($tab_globalobject, 'globalobject', $params).")
                    AND id_globalobject_to = ".$id_globalobject_origine."
                    ";
            }else{
                $sql_globalobject_link = "SELECT *
                    FROM    dims_globalobject_link
                    WHERE   id_globalobject_to IN (".$db->getParamsFromArray($tab_globalobject, 'globalobject', $params).")
                        AND id_globalobject_from = ".$id_globalobject_origine."
                    ";
            }

            $res_link = $db->query($sql_globalobject_link);

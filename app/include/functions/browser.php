...
function dims_detect_browser ($ua) {
	$os_ver = '';
	$os_name = '';
	$os_code = '';
	$pda_name = '';
	...
	if (preg_match('#MovableType[ /]([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		...
		$browser_ver = $matches[1];
		list($os_name, $os_code, $os_ver) = dims_unix_detect_os($ua);
		...
		if (preg_match('/Windows/i', $ua)) {
			list($os_name, $os_code, $os_ver) = dims_windows_detect_os($ua);
		} else {
			list($os_name, $os_code, $os_ver) = dims_unix_detect_os($ua);
		}
		...
		list($os_name, $os_code, $os_ver) = dims_unix_detect_os($ua);
		if (!$os_name) {
			list($os_name, $os_code, $os_ver) = dims_pda_detect_os($ua);
		}
		...
		$browser_ver = $matches[1];
		list($os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = dims_pda_detect_os($ua);
		...
		$browser_code = 'ie';
		$browser_ver = $matches[1];
		list($os_name, $os_code, $os_ver, $pda_name, $pda_code, $pda_ver) = dims_windows_detect_os($ua);
...
function dims_windows_detect_os ($ua) {
	$os_ver = '';
	$os_name = '';
	$os_code = '';
	$pda_name = '';
...
function dims_unix_detect_os ($ua) {
	$os_ver = '';
	$os_name = '';
	$os_code = '';
...
function dims_pda_detect_os ($ua) {
	$os_ver = '';
	$os_name = '';
	$os_code = '';
	$pda_code = '';

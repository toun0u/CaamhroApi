...
CKEDITOR.dialog.add( 'link', function( editor )
{
	...
	{
		var dialog = this.getDialog(),
			partIds = [ 'urlOptions','articleWikiOptions', 'articleWceOptions', 'anchorOptions', 'emailOptions' ],
	...
	if(editor.config.isWiki != undefined && editor.config.isWiki){
	return {
				...
				[
					{
						...
						[
							[ linkLang.toUrl, 'url' ],
                            [ 'Article WIKI', 'articleWiki' ],
                            [ 'Article WCE', 'articleWce' ],
                    ...
                    {
                        type : 'vbox',
                        id : 'articleWikiOptions',
                        children : [
                        {
                            type : 'select',
                            label : 'Article du Wiki',
                            id : 'article_wiki',
                            title : 'Article du Wiki',
                            items: jQuery.parseJSON(CKEDITOR.ajax.load('/admin-light.php?dims_op=wiki&op_wiki=sel_article_wiki')),
                            setup : function( data )
                            {
                                if ( data.localPage2 )
                                    this.setValue( data.localPage2 );
                            },
                            commit : function( data )
                            {
                                if ( !data.localPage2 )
                                    data.localPage2 = {};
                                data.localPage2 = this.getValue();
                            }
                        }]
                    },
                    {
                        type : 'vbox',
                        id : 'articleWceOptions',
                        children : [
                        {
                            type : 'select',
                            label : 'Article du WCE',
                            id : 'article_wce',
                            title : 'Article du WCE',
                            items: jQuery.parseJSON(CKEDITOR.ajax.load('/admin-light.php?dims_op=sel_article_wce')),
                            setup : function( data )
                            {
                                if ( data.localPage )
                            ...
                            onChange : function(){
                                var dialog = this.getDialog();
                                var select = dialog.getContentElement( 'info', 'section_wce' );
                                select.clear();
                                if (this.getValue() > 0){
                                    var objects = jQuery.parseJSON(CKEDITOR.ajax.load('/admin-light.php?dims_op=sel_section_wce&id='+this.getValue()));
                            ...
                            setup : function( data ){
                                if ( data.localPage ){
                                    ...
                                    if (data.localPage > 0){
                                        ...
                                        var select = dialog.getContentElement( 'info', 'section_wce' );
                                        select.clear();
                                        var objects = jQuery.parseJSON(CKEDITOR.ajax.load('/admin-light.php?dims_op=sel_section_wce&id='+data.localPage));
                                        for (var i in objects){
                                            var url = "&WCE_section_"+data.localPage+"_"+objects[i][2]+"="+objects[i][3]+"#"+objects[i][1];
                                            select.add(objects[i][0],url);
                                        }
                                        if ( data.sectionPage )
                                            select.setValue(data.sectionPage);
                                    }
                                }
                            },
                        },
                        {
                            ...
                            commit : function( data )
                            {
                                if ( !data.sectionPage )
                                    data.sectionPage = {};
                                data.sectionPage = this.getValue();
                            }
                        }]
                    },
					{
						...
						[
                            ...
                            {type:'button',
                            id:'browse',
                            filebrowser:{   action:'Browse',
                                            target:'info:url',
                                            url:CKEDITOR.config.filebrowserBrowseUrl},
                            style:'float:right',
		...
		onOk : function()
		{
			...
			switch ( data.type || 'url' )
			{
				...
				case 'anchor':
					...
					break;
                case 'articleWiki':
                    var id = data.localPage2;
                    if (id > 0)
                        attributes['data-cke-saved-href' ] ='index.php?articleid='+id;
                    break;
                case 'articleWce' :
                    var id = data.localPage;
                    if (id > 0){
                        attributes['data-cke-saved-href' ] ='index.php?articleid='+id;
                    	var sect = data.sectionPage;
                        if (sect != 0 && sect != '&WCE_section_'+id+'_1=1#0')
...
function applyTxtUrl(text){
    var dialog = CKEDITOR.dialog.getCurrent();
    dialog.setValueOf('info','url',text);
}
function switchProtocol(val){
    if(val == null) val = '';
    var dialog = CKEDITOR.dialog.getCurrent();
    var select = dialog.getContentElement('info', 'protocol');

...
function OpenFileBrowser( url, width, height )
{
    ...
    if (typeof fckDialogWindow.oEditor != 'undefined'){
        oEditor = fckDialogWindow.oEditor;
    }else if (typeof(fckDialogWindow.InnerDialogLoaded) == 'function'){
        oEditor = fckDialogWindow.InnerDialogLoaded();
    }else
        oEditor = new CKEDITOR.editor();

    if (oEditor != null){
        var iLeft = ( CKEDITOR.config.width  - width ) / 2 ;
        var iTop  = ( CKEDITOR.config.height - height ) / 2 ;
...
function applyTxtUrl(text){
    ...
    dialog.setValueOf('info','txtUrl',text);
}

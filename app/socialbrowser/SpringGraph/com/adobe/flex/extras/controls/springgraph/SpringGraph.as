 ...
 public class SpringGraph extends Canvas {
	    ...
	    /** @private */
 		public function newComponent(item: Item): UIComponent {
   	    ...
   	    protected function onEdge(event: MouseEvent):GraphEdge {
   	    	var edges: Array = _dataProvider.getEdges();
   	    	for each (var edge: GraphEdge in edges) {
   	    		var fnode:GraphNode = GraphNode (edge.getFrom());
   	    		var tnode:GraphNode  = GraphNode (edge.getTo());
				var f: UIComponent = fnode.view;
				var t: UIComponent = tnode.view;
				var fromX: int = f.x + (f.width / 2);
				var fromY: int = f.y + (f.height / 2);
				var toX: int = t.x + (t.width / 2);
				var toY: int = t.y + (t.height / 2);
				var rect:Rectangle = new Rectangle(fromX-2+(toX-fromX)/2, fromY-2+(toY-fromY)/2, 4,4);
	    ...
	    /** @private */
   		...
   		public function backgroundMouseDownEvent(event: MouseEvent):void  {
   			var now: int = getTimer();
   			if((now - lastMouseDownTime) < 300) {
   				// it's a double-click
   				//var node: GraphNode = _dataProvider.findNode(UIComponent(event.currentTarget));
   				...
   				return;
   					...
   					if (edge && this._edgeClicker){
   						_edgeClicker.edgeClick(edge, this._graph);
   						return;
   					}
		...
		*/
		...
		public function set edgeClicker(value:  IEdgeClicker):void{
			_edgeClicker = value;
		}
        ...
        protected var lastMouseDownTime: int = -999999;
        ...
        protected var _edgeClicker:IEdgeClicker=null;

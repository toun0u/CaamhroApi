...
public class GraphDataProvider implements IDataProvider {
	...
	public function set graph(g: Graph): void {
		...
		var data: Object = g.getLinkData(Item(edge[0]),Item(edge[1]));
			var weight: Number = (data != null) ? data.weight : 1.0;
			if (weight == 0 )
				weight = 1.0;
			edges.push(new GraphEdge(GraphNode(nodeStore[Item(edge[0]).id]), GraphNode(nodeStore[Item(edge[1]).id]), _distance, weight));
		}
	}

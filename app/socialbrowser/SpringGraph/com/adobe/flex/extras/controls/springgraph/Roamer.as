...
package com.adobe.flex.extras.controls.springgraph {
	...
	public class Roamer extends SpringGraph {
		...
		public function Roamer():void {
			super();
			 this.addEventListener("mouseDown", bgMouseDownEvent);
		}
		[Bindable]
		/**
		...
		public function get filterLinks(): Boolean{
			return _filterLinks;
		}
		public function set filterLinks(f : Boolean): void {
			_filterLinks = f;
			recreateGraph();
		}
		public function get linkFilters():Array  {
			return _linkFilters;
		}
		public function set linkFilters (fs: Array): void {
			_linkFilters =fs;
			recreateGraph();
		}
		/**
		...
		public function set maxDistanceFromCurrent(i: int): void {
			...
			recreateGraph();
			dispatchEvent(new Event("maxDistanceFromCurrentChange"));
			
		}
		
		[Bindable("maxDistanceFromCurrentChange")]
	...
	private function bgMouseDownEvent(event: MouseEvent):void  {
   			...
   			if((now - lastMouseDownTime) < 300) {
   				...
   				return;
   					...
   					var edge:GraphEdge = onEdge(event);
   					if (edge && this._edgeClicker){
   						_edgeClicker.edgeClick(edge, this.fullGraph);
   						return;
   					}
			...
			for (id in newItems) {
				...
				for(var neighborId: String in neighbors) {
					if(g.hasNode(neighborId)) {
						...
						//var data:Object = fullGraph.getLinkData(item,fullGraph.find(neighborId));
						var data:Object = canAddLink(item, fullGraph.find(neighborId));
						if (data)
							g.link(item, g.find(neighborId),data);
					}
				}
			}
			return newItems;
		...
		private function addToGraph(item: Item, generation: int, graph: Graph): Boolean {
			...
			if(generation < _maxDistanceFromCurrent) {
				...
				for(var neighborId: String in neighbors) {
					...
					// added for link filtering
					var data:Object = canAddLink(item, neighbor);
					if (data) {
		...
		private function canAddLink(item :Item, neighbor:Item):Object{
			var data:Object= fullGraph.getLinkData(item, neighbor);
			if (_filterLinks){
				if (data && data.type) {
				 for (var i:int =0; i< _linkFilters.length;i++){
				  if (_linkFilters[i] == data.type){
				  	return data;
				  }
				 }
				}
		...
		private var _filterLinks: Boolean = false;
		private var _linkFilters:Array = new Array();
	}
}

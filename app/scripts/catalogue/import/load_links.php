...
$input_dir = DIMS_APP_PATH.'data/synchro/input/links/';
...
foreach (glob($input_dir.'*.csv') as $filename) {
	...
	if (file_exists($filename)) {
		...
		$rs = $db->query('SELECT * FROM `'.$csv->getTableTemp().'` WHERE `column1` LIKE "Code article%" LIMIT 1');
		...
		$db->query('DELETE FROM `'.$csv->getTableTemp().'` WHERE `column1` LIKE "Code article%"');

		// On vire les lignes qui n'ont pas de lien
		$db->query('DELETE FROM `'.$csv->getTableTemp().'` WHERE `column3` = "" AND `column4` = ""');
		...
		while($fields = $db->fetchrow($rs)) {
			...
			$article = new article();
			if ($article->findByRef($fields['column1'])) {
				...
				$db->query('DELETE FROM `'.article_reference::TABLE_NAME.'`
					...
					AND 	`type` = '.article_reference::TYPE_URL);
				...
				if ($fields['column3'] != '') {
					$article_reference = article_reference::build(array(
						...
						'position' 		=> $position,
						'name' 			=> $title3,
						'type' 			=> article_reference::TYPE_URL,
						'url' 			=> $fields['column3']
						));

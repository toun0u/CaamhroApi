...
if (isset($exported_datas) && !empty($exported_datas)) {
	...
	foreach($exported_datas as $contact) {
		switch (get_class($contact)) {
			case 'contact':
				$i=0;
				$l++;
				$worksheet->writeString($l, $i++, utf8_decode($_SESSION['cste']['_DIMS_LABEL_CONTACT']), $format);
			...
			case 'tiers':
				$i=0;
				$l++;
				$worksheet->writeString($l, $i++, utf8_decode(ucfirst($_SESSION['cste']['COMPANY_MINUSCULE'])), $format);
				$worksheet->writeString($l, $i++, '', $format);
				$worksheet->writeString($l, $i++, '', $format);
				$worksheet->writeString($l, $i++, '', $format);
				$worksheet->writeString($l, $i++, '', $format);

...
switch ($action) {
	default:
	case 'show':
		require_once _DESKTOP_TPL_LOCAL_PATH.'/admin/tag/display_tag.tpl.php';
		break;
	case 'save_label':
		ob_clean();
		...
		$tag = tag::find_by(array('id_workspace'=>$_SESSION['dims']['workspaceid'],'id'=>$id, 'type'=>tag::TYPE_DEFAULT),null,1);
	...
	case 'save':
		...
		dims_redirect(dims::getInstance()->getScriptEnv()."?submenu=1&mode=admin&o=tag");
		break;
	case 'delete':
		$id = dims_load_securvalue("id",dims_const::_DIMS_NUM_INPUT,true,true,true);
		$tag = tag::find_by(array('id_workspace'=>$_SESSION['dims']['workspaceid'],'id'=>$id, 'type'=>tag::TYPE_DEFAULT),null,1);

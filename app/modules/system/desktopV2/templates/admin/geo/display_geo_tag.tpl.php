...
?>
	<form method="POST" action="<?= dims::getInstance()->getScriptEnv(); ?>?submenu=1&mode=admin&o=geo&action=save" name="form_tag" id="form_tag">
	<table cellpadding="0" cellspacing="0" class="tag-categ-obj">
		...
		foreach($tags as $t){
			...
			<tr>
				...
				<td>
					<a href="<?= dims::getInstance()->getScriptEnv()."?submenu=1&mode=admin&o=geo&action=edit&id=".$t->get('id'); ?>"><img src="<?= _DESKTOP_TPL_PATH; ?>/gfx/contact/pencil.png" /></a>
					<a href="javascript:void(0);" onclick="javascript:dims_confirmlink('<?= dims::getInstance()->getScriptEnv()."?submenu=1&mode=admin&o=geo&action=delete&id=".$t->get('id'); ?>','<?= $_SESSION['cste']['ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_ELEMENT_?']; ?>');"><img src="<?= _DESKTOP_TPL_PATH; ?>/gfx/contact/remove_black.png" /></a>
				</td>
			</tr>
			<?php
		}
		...
		<tr>
			...
			?>
			<td><input type="radio" checked=true name="new_obj" value="0" /></td>
			<td></td>
		</tr>
	</table>
	<div class="actions" style="float:none;">
		...
		<a href="<?= dims::getInstance()->getScriptEnv()."?submenu=1&mode=admin&o=geo"; ?>">

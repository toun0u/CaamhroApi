...
<div class="title_description">
	...
	<h2><? echo $_SESSION['cste']['_DIMS_LABEL_STEP']; ?> 1 - <?php echo $_SESSION['cste']['EVENT_LINKED_TO_THE_BUSINESS_ACTIVITY']; ?></h2>
</div>
<table class="date_of_activity" cellspacing="6" cellpadding="0">
	<tbody>
		<tr>
			<td class="text">
				<input type="hidden" id="selected_type_event" name="selected_type_event" value="none" />
				<input type="radio" id="type_event_trade_missions" name="type_event" value="trade_missions" /> <label class="radio_label" for="type_event_trade_missions"><?php echo $_SESSION['cste']['_TRADE_MISSIONS']; ?></label>
				<input type="radio" id="type_event_trade_fairs" name="type_event" value="trade_fairs" /> <label class="radio_label" for="type_event_trade_fairs"><?php echo $_SESSION['cste']['_TRADE_FAIRS']; ?></label>
				<input type="radio" id="type_event_none" name="type_event" value="none" checked="checked" /> <label class="radio_label" for="type_event_none"><?php echo $_SESSION['cste']['_NEW_ACTIVITY_WITHOUT_LINK_TO_EXISTING_EVENT']; ?></label>
			</td>
		</tr>
		<tr>
			<td class="text">
				<div id="events_list">
					...
					<input type="text" class="activity_field" value="" onkeyUp="javascript:filterEventActivity();" id="filtereventlabel" />

					<select name="link" id="link" class="link activity_field" onchange="javascript:addEventInActivity($(this).val());">
						<option value=""></option>
					</select>
					<a id="removeEventLink" class="button" href="Javascript: void(0);" onclick="javascript:removeEventFromActivity();">
...
<div class="title_description">
	<img src="<?php echo _DESKTOP_TPL_PATH; ?>/gfx/common/home_cube.png" border="0" />
	<h2><? echo $_SESSION['cste']['_DIMS_LABEL_STEP']; ?> 2 - <?php echo $_SESSION['cste']['DATE_OF_THE_ACTIVITY']; ?></h2>
</div>
<div class="date_of_activity">
	<div class="table_description_gauche">
		<p id="activity_date_from">
			<span class="date_label"><? echo $_SESSION['cste']['_DIMS_DATE']; ?> <font style="color:#df1d31;"> *</font></span>
			<input maxlength="2" name="datestart_day" id="datestart_day" class="activity_field" style="width: 32px;" value="<?php if (!empty($datestart)) echo $datestart[2]; else echo date('d'); ?>" /> /
			<input maxlength="2" name="datestart_month" id="datestart_month" class="activity_field" style="width: 32px;" value="<?php if (!empty($datestart)) echo $datestart[1]; else echo date('m'); ?>" /> /
			<input rel="requis" maxlength="4" name="datestart_year" id="datestart_year" class="activity_field" style="width: 50px;" value="<?php if (!empty($datestart)) echo $datestart[0]; else echo date('Y'); ?>" />
...
<div class="title_description">
	<img src="<?php echo _DESKTOP_TPL_PATH; ?>/gfx/common/home_cube.png" border="0" />
	<h2><? echo $_SESSION['cste']['_DIMS_LABEL_STEP']; ?> 3 - <?php echo $_SESSION['cste']['LOCATION_OF_THE_ACTIVITY']; ?></h2>
</div>
<div class="location_of_activity">
		...
		<tbody>
			<tr>
				...
				<td style="width: 300px;">
					<select rel="requis" style="width: 300px;" name="activity_country" class="activity_country" id="activity_country" data-placeholder="<?php echo $_DIMS['cste']['_DIMS_SELECT_COUNTRY']; ?>">
						<option value=""></option>
						<?php
						$sel_Country = null;
				...
				<td class="activity_rech_add_city" id="activity_rech_add_city" style="width: 204px">
					<select id="city_activity" type="text" name="activity_city" class="city_activity" <?php echo ($sel_Country != null && $sel_Country->fields['id'] > 0) ? '' : 'disabled="disabled"'; ?> style="width:198px;" data-placeholder="<?php echo $_DIMS['cste']['_DIMS_SELECT_CITY']; ?>">
						<option value=""></option>
						<?
	...
	<div class="table_description_gauche">
		<table cellspacing="10" cellpadding="0" style="float:left;">
			<tbody>
				<tr>
					<td class="type_ie7">
						<?php echo $_SESSION['cste']['LABEL_OF_THE_ACTIVITY']; ?>
					</td>
					<td style="text-align:left;">
						<input id="label" type="text" name="label" value="<?php echo stripslashes($this->fields['libelle']); ?>" style="width: 240px;" class="activity_field" />
					</td>
				</tr>
                <tr>
					<td class="type_ie7">
					...
					<td id="activity_rech_add_type" style="text-align:left;">
						<select style="width: 240px;" id="type_id" name="type_id" class="type" data-placeholder="<?php echo $_DIMS['cste']['_DIMS_SELECT_TYPE']; ?>">
							...
							if (sizeof($a_types)) {
								foreach ($a_types as $type) {
									$sel = ($type->fields['id'] == $this->fields['activity_type_id']) ? ' selected' : '';
...
<?php /*
				<tr>
					...
					<td style="text-align:left;float: left; margin-top: 4px; width:100%;" id="activity_rech_add_sector">
						<select data-placeholder="<?php echo $_DIMS['cste']['_DIMS_SELECT_SECTOR']; ?>" id="sector_id" name="sector_id" style="width: 240px;" class="sector">
							...
							if (sizeof($a_sectors)) {
								foreach ($a_sectors as $sector) {
									$sel = ($sector->fields['id'] == $this->fields['activity_sector_id']) ? ' selected' : '';
							...
							<tr>
								<td>
									...
									if ($this->fields['banner_path'] != '' && file_exists($this->fields['banner_path']))
										echo '<img class="activity_img" src="'.$this->fields['banner_path'].'" alt="'.$_SESSION['cste']['_AVATAR'].'" title="'.stripslashes($this->fields['libelle']).'" />';
									?>
									</div>
								</td>
								<td>
									<div class="text-avatar"><?php echo $_SESSION['cste']['_AVATAR']; ?></div>
								</td>
								<td>
                ...
                <tr>
                    ...
                    <td colspan="2" style="width:80%;">
                        <div style="float:right; width: 100%;">
                            <textarea id="description" name="description" class="activity_field activity_textarea"><?php echo stripslashes(str_replace('\r\n', "\r\n", $this->fields['description'])); ?></textarea>
                        </div>
                    </td>
                </tr>
            </tbody>
...
<script language="javascript" type="text/javascript">
    $(document).ready(function(){
        $("#date_range").click(function() {
			$("#activity_date_to").fadeToggle('fast',function(){
				if($("#activity_date_from span:first").html() == '<? echo $_SESSION['cste']['_DIMS_DATE']; ?> <font style="color:#df1d31"> *</font>')
					$("#activity_date_from span:first").html('<? echo $_SESSION['cste']['_FROM']; ?> <font style="color:#df1d31"> *</font>');
				else
					$("#activity_date_from span:first").html('<? echo $_SESSION['cste']['_DIMS_DATE']; ?> <font style="color:#df1d31"> *</font>');
			});
        });

		if ($('img.activity_img').length){
			$('img.activity_img').css("margin-left",2-($('img.activity_img').css("width").substr(0,$('img.activity_img').css("width").length-2))+"px");
			$('img.activity_img').css("margin-top","-2px");
		}
		$("select.city_activity").chosen({allow_single_deselect:true, no_results_text: "<div onclick=\"javascript:addNewCity('activity_rech_add_city','activity_country');\" style=\"float:right;color:#E21C2C;cursor:pointer;\"><img style=\"float:left;\" src=\"<? echo _DESKTOP_TPL_PATH; ?>/gfx/common/add.png\" /><div style=\"float:right;margin-top:3px;\">Add it !</div></div>No results matched"});
		$("select.activity_country")
		...
		$("select#sector_id").chosen({allow_single_deselect:true, no_results_text: "<div onclick=\"javascript:addNewSector('activity_rech_add_sector');\" style=\"float:right;color:#E21C2C;cursor:pointer;\"><img style=\"float:left;\" src=\"<? echo _DESKTOP_TPL_PATH; ?>/gfx/common/add.png\" /><div style=\"float:right;margin-top:3px;\">New sector ?</div></div>No results matched"});
		$("select#type_id").chosen({allow_single_deselect:true, no_results_text: "<div onclick=\"javascript:addNewType('activity_rech_add_type');\" style=\"float:right;color:#E21C2C;cursor:pointer;\"><img style=\"float:left;\" src=\"<? echo _DESKTOP_TPL_PATH; ?>/gfx/common/add.png\" /><div style=\"float:right;margin-top:3px;\">New type ?</div></div>No results matched"});

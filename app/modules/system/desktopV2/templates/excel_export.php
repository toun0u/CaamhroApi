...
if (!empty($lstContacts)) {
		...
		// $sql = "select * from dims_mod_business_contact where id in (".$db->query($lstContacts, 'idcontact', $params).")"; <= Ligne originale /!\ A TESTER
		$sql = "select * from dims_mod_business_contact where id in (".$this->db->getParamsFromArray(explode(',', $db->query($lstContacts, 'idcontact', $params) ), 'contacts', $params).")";
	...
	foreach ($lstContacts as $contact) {
		...
		foreach ($_SESSION['business']['exportdata'] as $col => $field) {
			...
			$worksheet->writeString($line, $col, utf8_decode($contact[$field['namefield']]), $format);
		}
	}

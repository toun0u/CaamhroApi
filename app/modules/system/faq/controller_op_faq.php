...
switch($action){
	...
	case 'edit':
		...
		if ($AddCateg){
			...
			?>
			<span style="width:100%;margin-bottom:10px;" id="arianeCateg_<? echo $id; ?>">
			<?
			if (count($link) > 0){
				$categ = new category();
				$categ->openWithGB(current($link));
				echo 'Cat&eacute;gorie : '.$categ->getAriane();
				echo dims_create_button("Changer cat&eacute;gorie",'','javascript:publiFaqAddCateg('.$id.');','','float:right;');
			}else{
				echo 'Aucune cat&eacute;gorie';
				echo dims_create_button("Choisir cat&eacute;gorie",'','javascript:publiFaqAddCateg('.$id.');','','float:right;');
			}
			?>
			</span>
			<?
		...
		?>
		<form method="POST" action="<? echo $dims->getScriptEnv(); ?>" name="saveFaq_<? echo $id; ?>">
			<?
			...
			if ($AddCateg){
				?>
				<input type="hidden" name="idCateg" id="idCateg_<? echo $id; ?>" value="<? if (count($link) > 0) echo current($link); else echo 0; ?>" />
				<?
				$token->field("idCateg", ( (count($link) > 0)?current($link):echo 0 );
			}
			dims_fckeditor('lang'.$id.'_content', $faq->getContent(), '100%', 290,true);
			?>
		...
		<?
		echo dims_create_button($_SESSION['cste']['_DIMS_SAVE'],'','javascript:if(document.saveFaq_'.$id.'.langFaq_title.value != \'\') document.saveFaq_'.$id.'.submit(); else document.saveFaq_'.$id.'.langFaq_title.style.backgroundColor=\'#FF0000\';','','float:right;','javascript:void(0);');
		echo dims_create_button($_SESSION['cste']['_DIMS_LABEL_CANCEL'],'','javascript:dims_xmlhttprequest_todiv(\'admin.php\',\'dims_op=dims_faq_manager&action=view&id='.$id.'\',\'\',\'content_'.$id.'\');','','float:right;');
		echo dims_create_button($_SESSION['cste']['_DELETE'],'','javascript:dims_confirmlink(\'?dims_op=dims_faq_manager&action=delete&id='.$id.'\',\'Êtes-vous sur de vouloir supprimer ce FAQ ?\');','','float:right;');
		break;
	case 'view':
		$id = dims_load_securvalue('id',dims_const::_DIMS_NUM_INPUT,true,true,true);
		if ($id != '' && $id > 0){
	...
	case 'viewCateg' :
		...
		$res = $db->query($sel, array(
			...
			':idobject' => dims_const::_SYSTEM_OBJECT_FAQ
		));
		$lstCat = array();
		$lvl = 0;
		...
		$browser = new dims_browser($lvl+1,$lstCat,'listeCategFaqSave');
		?>
		<div id="bank_account_detail">
			...
			<?
			if ($id != '' && $id > 0)
				echo dims_create_button($_SESSION['cste']['_DIMS_ADD'],'','javascript:dims_xmlhttprequest_todiv(\'admin.php\',\'dims_op=dims_faq_manager&action=refreshCateg&id='.$id.'&idCateg=\'+document.getElementById(\'popupidCateg\').value,\'\',\'arianeCateg_'.$id.'\');dims_closeOverlayedPopup('.$id_popup.');','','float:right;margin-bottom:10px;margin-right:20px;');
			else
				echo dims_create_button($_SESSION['cste']['_DIMS_ADD'],'','javascript:dims_xmlhttprequest_todiv(\'admin.php\',\'dims_op=dims_faq_manager&action=refreshCateg&id='.$id.'&idCateg=\'+document.getElementById(\'popupidCateg\').value,\'\',\'arianeCateg_X\');dims_closeOverlayedPopup('.$id_popup.');','','float:right;margin-bottom:10px;margin-right:20px;');
		...
		<script type="text/javascript">
			function initCateg(elem,parent){
				...
				$("div.listeCategFaqSave li[class~='browser']").click(function(){initCateg($(this),$(this).parents("div:first"));});
			}

			<? if (isset($_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_lvl']) && isset($_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_id'][$_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_lvl']])){ ?>
				var selected = $("div.listeCategFaqSave li[class~='selected'][name*='_<? echo $_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_id'][$_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_lvl']]; ?>']") ;
				var prt = selected.parents("div#<? echo $_SESSION['dims']['dims_browser']['listeCategFaqSave']['selected_lvl']; ?>");
	...
	case 'refreshCateg' :
		...
		?>
		<script type="text/javascript">
			document.getElementById('idCateg_<? if ($id != '' && $id > 0) echo $id; else echo 'X'; ?>').value = <? echo $idCateg; ?>;
		</script>
		<?
		break;
}

<?php
	echo $skin->open_widgetbloc($_DIMS['cste']['_SHARE'], 'width:100%;', 'font-weight:bold;padding-bottom:2px;padding-left:10px;vertical-align:bottom;', './common/img/widget_share.png','26px', '26px', '-17px', '-5px', '', '', '');
	$arrayshare=array();
	...
	if (isset($contact->fields['id']) && $contact->fields['id']>0) {
		...
		$arrayshare=$dims->getShares(1, dims_const::_SYSTEM_OBJECT_CONTACT, $id_record);
	}
	...
	if (empty($arrayshare)) {
		...
		echo '<tr class="trl1">
                ...
                <td style="width: 85%;">'.$_DIMS['cste']['_DIMS_LABEL_ORIGIN'].'</td>';
		...
		foreach($arrayshare as $k=>$sh) {
                        ...
                        if ($sh['type_from']==0 && isset($works[$sh['id_from']]) || $sh['type_from']==1) {
                            ...
                            /*if ($_SESSION['dims']['workspaceid']==$sh['id_from']) {
                                            echo "<td><a href=\"javascript:dims_confirmlink('".dims_urlencode($scriptenv."?cat="._BUSINESS_CAT_CONTACT."&op=deletepublicshare&action="._BUSINESS_TAB_CONTACT_FORM."&id_work=".$_SESSION['dims']['workspaceid']."&id_ct=".$id_contact."&part="._BUSINESS_TAB_CONTACT_IDENTITE)."','".$_DIMS['cste']['_DIMS_CONFIRM']."');\"><img src=\"./common/img/delete.png\" alt=\"".$_DIMS['cste']['_DELETE']."\"></a></td>";
                            }*/
                            echo '</tr>';
                        }
		}

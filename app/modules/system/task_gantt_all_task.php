...
define("_COLOR_OK","#9ae8b0");
define("_COLOR_NON_OK","#ff6868");
define("_COLOR_CLASSIC","#75c5e2");
...
/* Le tableau des mois */
$month_array = array($_DIMS['cste']['_JANUARY'],$_DIMS['cste']['_FEBRUARY'],$_DIMS['cste']['_MARCH'],$_DIMS['cste']['_APRIL'],$_DIMS['cste']['_MAY'],$_DIMS['cste']['_JUNE'],$_DIMS['cste']['_JULY'],$_DIMS['cste']['_AUGUST'],$_DIMS['cste']['_SEPTEMBER'],$_DIMS['cste']['_OCTOBER'],$_DIMS['cste']['_NOVEMBER'],$_DIMS['cste']['_DECEMBER']);
...
$sql = "select min(concat(right(t.date_start, 4),'-',mid(t.date_start, 4, 2),'-',left(t.date_start, 2))) as min_date,
    ...
    from dims_mod_prjt_task t, dims_mod_prjt_project p, dims_mod_prjt_task_user u, dims_mod_prjt_project_task pt
    where u.id_task = t.id and t.id = pt.id_task and pt.id_project = p.id and u.id_user = :iduser";
...
$id_p_previous="-1";
while($fields = $db->fetchrow($res)) {
     if($fields['p.id']!=$id_p_previous) {
	  $h+=20; /* une bande */
	  $id_p_previous=$fields['id'];
     }
     $h+=5; /* La taille d'un t�che */
}

/* On selectionne toutes les t�ches li�es a l'utilisateur connect� (dans l'ordre chronologique) */
$sql2 = "select distinct t.*, p.*
    from dims_mod_prjt_task t, dims_mod_prjt_project p, dims_mod_prjt_task_user u, dims_mod_prjt_project_task pt
    where u.id_task = t.id and t.id = pt.id_task and pt.id_project = p.id and u.id_user = :iduser order by concat(right(t.date_start, 4),'-',mid(t.date_start, 4, 2),'-',left(t.date_start, 2))";
...
while($fields = $db->fetchrow($res)) {
	...
	$nb_jour_total = $view_nb_jour['nb_day']+$start_nb_jour+$end_nb_jour;
	 ...
	 $sql2 = "select distinct t.*
	     from dims_mod_prjt_task t, dims_mod_prjt_project p, dims_mod_prjt_task_user u, dims_mod_prjt_project_task pt
	     where u.id_task = t.id and t.id = pt.id_task and pt.id_project = :idproject and u.id_user = :iduser order by concat(right(t.date_start, 4),'-',mid(t.date_start, 4, 2),'-',left(t.date_start, 2))";

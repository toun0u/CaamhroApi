...
if (isset($_SESSION['dims']['current_object']['id_record']) && is_numeric($_SESSION['dims']['current_object']['id_record'])) {
	...
	$sqlg = "SELECT			distinct t.id,
				...
				ON			l.id_tag = t.id
				AND			l.id_module_type= :idmoduletype
				AND			l.id_tag = t.id
	...
	if($db->numrows($resg) > 0) {
		...
		while($tab_group = $db->fetchrow($resg)) {
			...
			$arraytypetag[$tab_group['type']]['nb']++;
		}
	...
	if($db->numrows($resu) > 0) {
		...
		while($tab_u = $db->fetchrow($resu)) {
			...
			if ($tab_u['private'] == 0){
				$add_g.= "	<li style='margin:1px'>
								<input type='checkbox' name='cttoadd[]' value='".$tab_u['id']."' $check onclick=\"javascript:addSelectedTag('".$tab_u['id']."',".$typetag.",".$boolcheck.");\">".$tab_u['label'];
				$token->field("cttoadd");
					if ($typetag <=3)
		...
		if ($private == '')
			...
			$add_g .= '	</ul>
						<span style="float:left;"><b>'.$_DIMS['cste']['_PRIVATE'].'</b></span>
						<table width="100%">
							<tr>
								'.$private.'
							</tr>
		...
		$add_g .= "</div><div style=\"clear:both;\">";
	...
	// ajouter des test pour enlever : geographic area (3) et year (4) dims_isadmin() &&
		if ($typetag < 3)

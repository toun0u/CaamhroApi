...
$type_doc=substr($doc->fields['name'],0,strlen($fields['name'])-4);
...
// construction des tags � remplacer
...
$xmlmodel->addtag('(DATEFIN_INSC)', $date_fin_etap['date']);
...
if (is_writable($output_path)) {
	...
	if (fwrite($handle, $styles) === FALSE) {
		echo "Impossible d'ecrire dans le fichier $modele_styles";
		exit;
	}
	...
	if (fwrite($handle, $content) === FALSE) {
		echo "Impossible d'ecrire dans le fichier $modele_content";
		exit;
	}
	...
	shell_exec(escapeshellcmd("zip -r ".escapeshellarg("../$output_odt")." *"));
	...
	if ($format != 'ODT') {
		...
		//if (isset(_DIMS_JAVAPATH)) $javapath=_DIMS_JAVAPATH;
		...
		if (!file_exists(realpath($output_path)."/".$output_file)) {
			//$cmd = " unoconv -f pdf ".realpath("{$output_path}/{$output_odt}");

			// OVH
			$cmd = "/usr/bin/java -jar ".escapeshellarg("$converter_path")." ".escapeshellarg(realpath("{$output_path}/{$output_odt}")).' '.escapeshellarg(realpath("{$output_path}")."/{$output_file}");

			shell_exec($cmd);
		}
		dims_downloadfile($output_path.$output_file,$output_file, true, true);
	}
	else dims_downloadfile($output_path."/".$output_odt, $output_odt, true, true);
}
else {
	echo "Le dossier $output_path n'est pas accessible en �criture.";
}

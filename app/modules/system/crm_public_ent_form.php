...
$sql=	"SELECT dims_mod_business_meta_field.id,mb.label
		...
		AND (mb.name='intitule')
		ORDER BY dims_mod_business_meta_field.name desc";
...
// ajout des champs devant etre renseignes pour l'ajout
...
/*
$sql="SELECT * FROM `dims_constant` WHERE `phpvalue` LIKE '%_DIMS_NACE_%'";

$res=$db->query($sql);

if ($db->numrows($res)>0) {
	while ($f=$db->fetchrow($res)) {
		$code = substr($f['phpvalue'], 11, 3);
		$q="insert into dims_mod_business_enum set type='codenace',libelle='".dims_sql_filter($f['value'])."',lang=".$f['id_lang'].",code='".dims_sql_filter($code)."'";
		$resu=$db->query($q);
	}
}

// on fait maintenant les pays
$sql = "SELECT * FROM dims_constant WHERE moduletype LIKE 'system' AND phpvalue LIKE '_DIMS_PAYS%'";
$res=$db->query($sql);

if ($db->numrows($res)>0) {
	while ($f=$db->fetchrow($res)) {
		$q="insert into dims_mod_business_enum set type='pays',libelle='".dims_sql_filter($f['value'])."',lang=".$f['id_lang'];
		$resu=$db->query($q);
	}
}*/
...
<script language="javascript" type="text/javascript">
	...
	window['checkUpdateCateg'] = function checkUpdateCateg(id_cat) {
		id_categ=id_cat;
		var elem=document.form_ent['ch_categ_'+id_categ];
		value=-1;
	...
	window['validate_form_ent'] = function validate_form_ent(id) {
		<?
	...
	window['validate_form_ent_new'] = function validate_form_ent_new(id) {
		if(validate_dyn_field()) {
			if(id == 0) {
				var popup = document.getElementById("inf_similarite_ent");
				popup.style.display="block";
				verif_similar(document.getElementById("field<? echo $tabfiltrefield[0]['id']; ?>").value);
				document.getElementById('inf_similarite_ent').focus();
			}
			else {
				document.form_ent.submit();
			}
	...
	window['verif_similar'] = function verif_similar(intitule) {
		id_popup = dims_openOverlayedPopup(500,424);
		dims_xmlhttprequest_todiv("admin.php", "dims_op=verif_similar_ent&raisoc="+intitule+"&id_popup="+id_popup,"",'p'+id_popup);
	}
	<?
...
$_SESSION['dims']['tiers_fields_view']=array();
...
while ($fields = $db->fetchrow($rs_fields)) {
	...
	$_SESSION['dims']['tiers_fields_view'][$fields['id']]=0; // public ou workspace ou user
...
if (!isset($_SESSION['dims']['crm_newent_actionform'])) {
	$action_formsave="";
	$marginleft="250px";
}
else {
	$action_formsave = $_SESSION['dims']['crm_newent_actionform'];
	$marginleft="0px";
}
if($ent->fields['inactif'] != 1) {
?>

<form action="<? echo $action_formsave;?>" method="post" enctype="multipart/form-data" id="form_ent" name="form_ent">
<?
	// Sécurisation du formulaire par token
	require_once DIMS_APP_PATH . '/include/class_form_tokenizer.php';
	$token = new FormToken\TokenField;
?>
<div id="inf_similarite_ent" style="position:absolute;display:none;z-index:20;margin-top:150px;margin-left:<? echo $marginleft;?>;width:450px;height:350px;"></div>
<?php
...
if(!empty($_SESSION['dims']['desktopv2']['matrice']['id_from']) && !empty($_SESSION['dims']['desktopv2']['matrice']['type_from']) && $_SESSION['dims']['desktopv2']['matrice']['type_from'] == dims_const::_SYSTEM_OBJECT_CONTACT) {
	$retour = '<table width="100%" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td width="35%" align="left">
				<select id="tiers_type_link" name="ent_pers_type_link" style="background-color:#ebf2ea;">
					<option value="'.$_DIMS['cste']['_DIMS_LABEL_EMPLOYEUR'].'">'.$_DIMS['cste']['_DIMS_LABEL_EMPLOYEUR'].'</option>
		...
		<tr>
			...
			<td width="35%" align="left">
				<select id="tiers_link_level" name="ent_pers_link_level" style="background-color:#ebf2ea;">
					<option value="1">'.$_DIMS['cste']['_DIMS_LABEL_LFB_GEN'].'</option>
	...
	echo $retour;
	$token->field("ent_pers_type_link");
	$token->field("ent_pers_link_level");
...
if (isset($ent->fields['id']) && $ent->fields['id']>0) {
	...
	$sqlshare = "select		*
				...
				OR			(type_share=2 and id_from= :iduser )";
...
foreach($ent->fields as $name=>$val) {
	if ($val!="" && isset($convmeta[$name])) {
		$idmeta=$convmeta[$name];

		if (!$shareactive && $_SESSION['dims']['tiers_fields_mode'][$idmeta]==1 && $ent->fields[$name]!=''
			&& $name != 'intitule') {
			// partageable et valeur deja saisie
			if ($ent->fields['id_workspace']==$_SESSION['dims']['workspaceid']) {
				...
				$share->fields['id_record']=$ent->fields['id'];
			...
			$nbsharefield++;
			// on met en workspace
			if ($_SESSION['dims']['tiers_fields_view'][$idmeta]==0) $_SESSION['dims']['tiers_fields_view'][$idmeta]=1;
		}
	}
}

// on duplique les vues pour verification des changements
$_SESSION['dims']['tiers_fields_view_old']=$_SESSION['dims']['tiers_fields_view'];

/**********************************************************************************************/
// construction des donnes de partages
/**********************************************************************************************/
if($id_from != 0) {
...
?>
	...
	<tr>
		<td width="45%" style="vertical-align:top;">
			...
			$lstworkpaces=$workspace->getWorkspaceShareObject(dims_const::_SYSTEM_OBJECT_TIERS);
			...
			*/
				...
				<tr>
					...
					if (!empty($lstworkpaces)) {
						...
						<table style="width:100%;border:1px dotted">
							...
							<tr><td style="width:5%" valign="top"><img src="./common/img/all.png"></td><td style="text-align:left;"><? echo $_DIMS['cste']['_DIMS_LABEL_PUBLIC'];?></td>
							<td style="width:5%" valign="top"><img src="./common/img/share.png"></td><td style="text-align:left;"><? echo $_DIMS['cste']['_SHARE'];?></td>
							<td style="width:5%" valign="top"><img src="./common/img/users.png"></td><td style="text-align:left;"><? echo $_DIMS['cste']['_WORKSPACE'];?></td>
							<td style="width:5%" valign="top"><img src="./common/img/user.png"></td><td style="text-align:left;"><? echo $_DIMS['cste']['_PRIVATE'];?></td></tr>
						</table>
						</td>
					<?
					}
					...
					<?php
						...
						if(!empty($ent->fields['id']))
							echo dims_create_button($_DIMS['cste']['_DIMS_SAVE'],"./common/img/save.gif","javascript:validate_form_ent('".$ent->fields['id']."');","","");
						else {
							if (!empty($ent->fields['id'])) $ctid=$ent->fields['id'];
							else $ctid=0;
							echo dims_create_button($_DIMS['cste']['_DIMS_SAVE'],"./common/img/save.gif","javascript:validate_form_ent_new('".$ctid."');","","");
						}
			...
			while ($fields = $db->fetchrow($rs_fields)) {
				...
				if ($categcour!=$fields['id_metacateg']) {
					...
					?>
					<table style="width:100%;height:20px;padding-top:0px;" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td style="width:16%;text-align:left;">
									...
									<table align="left" cellspacing="0" cellpadding="0" style="text-align:left;width:100%;">
										<tr>
											<td style="width:32px;text-align:center;" valign="top"><img src="./common/img/all.png"> <img src="./common/img/share.png">
											</td>
											<td style="width:32px;text-align:center;" valign="top"><img src="./common/img/users.png">
											</td>
											<td style="width:32px;text-align:center;" valign="top"><img src="./common/img/user.png">
											</td>
										</tr>
										<tr>
											<td style="text-align:center;">
												<input type="radio" style="width:32px;" id="ch_categ_<? echo $ind_categ;?>" name="ch_categ_<? echo $ind_categ;?>" onclick="javascript:checkUpdateCateg(<? echo $ind_categ;?>)">
											</td>
				...
				if (isset($rubgen[$fields['id_metacateg']]['list'][$fields['id']]['use']) && $rubgen[$fields['id_metacateg']]['list'][$fields['id']]['use']>0) {
					...
					if ($ent->fields[$fields['namefield']]=='') {
						// if mode = partage, par defaut sur workspace au cas ou
						if (isset($_SESSION['dims']['tiers_fields_view'][$fields['id']]) && $_SESSION['dims']['tiers_fields_view'][$fields['id']]==0 && $_SESSION['dims']['tiers_fields_mode'][$fields['id']]>0) {
							$_SESSION['dims']['tiers_fields_view'][$fields['id']]=1;
							$_SESSION['dims']['tiers_fields_view_old'][$fields['id']]=1;
						}
					}

					if (isset($_SESSION['dims']['tiers_fields_view'][$fields['id']])) {
						switch ($_SESSION['dims']['tiers_fields_view'][$fields['id']]) {
							case 0 : // public
								$replies[$fields['id']] = explode('||',$ent->fields[$fields['namefield']]);
								break;
							case 1 : // workspace
								$replies[$fields['id']] = explode('||',$entworkspace->fields[$fields['namefield']]);
								break;
							case 2 : // private
								$replies[$fields['id']] = explode('||',$entuser->fields[$fields['namefield']]);
								break;
						}
					}
					else {
					...
					// verification de la valeur par d�faut si nouveau contact
					if ($ent->fields['id']==0 || $ent->fields['id']=='' || ($ent->fields['id']>0 && $replies[$fields['id']][0]=='')) {
					...
					if ($fields['enum']!='') {
						$sql_sa = "SELECT * FROM dims_mod_business_enum WHERE type LIKE :type AND lang = :lang ";
					...
					if (isset($_SESSION['dims']['tiers_fields_view'][$fields['id']])) {
						switch ($_SESSION['dims']['tiers_fields_view'][$fields['id']]) {
							case 0 : // public
								$colorfield='background-color:#ebf2ea;';
								break;
							case 1 : // workspace
					...
					if ($fields['namefield'] == 'pays'){
						?>
						<select style="width: 300px;" name="id_country" class="crm_country" id="crm_country" data-placeholder="<?php echo $_SESSION['cste']['_DIMS_SELECT_COUNTRY']; ?>">
							<?
							...
							if ($ent->fields['id_country'] == 0 || $ent->fields['id_country'] == ''){
								$ent->updateIdCountry();
							}
							$sel_Country = null;
							foreach (country::getAllCountries() as $country) {
								$sel = '';
								if ($country->fields['id'] == $ent->fields['id_country']){
									$sel = "selected=true";
									$sel_Country = $country;
								}
						...
						<?
					}/*elseif ($fields['namefield'] == 'ville'){
						?>
						<select style="width: 300px;" name="crm_country" class="crm_country" id="crm_country" data-placeholder="<?php echo $_SESSION['cste']['_DIMS_SELECT_COUNTRY']; ?>">
							<option value=""></option>
						</select>
						<?
					}*/else
					...
					if (!empty($lstworkpaces) && $fields['namefield']!='firstname' && $fields['namefield']!='lastname'	&& $fields['namefield']!='photo') {
						...
						else $value_empty=false;

						// affichage des vues eventuels sur les donnees
						if (isset($_SESSION['dims']['tiers_fields_view'][$fields['id']])) {
							for ($c=0;$c<=2;$c++) {
								$link="";
								$sel=($_SESSION['dims']['tiers_fields_view'][$fields['id']]==$c);
								...
								switch ($c) {
									...
									case 1 : // workspace
										...
										if ($sel) {
											...
											if ($_SESSION['dims']['tiers_fields_view'][$fields['id']]==0) {
												$src="./common/img/bullet.png";
											}
											else {
												if (!is_array($replies[$fields['id']]) && $replies[$fields['id']] !='' || (is_array($replies[$fields['id']]) && $replies[$fields['id']][0]!='')) {
					...
					// on regarde si on a des donn�es partagees et que l'on ne peut les voir
					if (!$shareactive && $nbsharefield>0) {
						if ($ind_field==0) {
							...
							echo "Il existe des champs partag�s, acceptez vous le partage ?";
							echo dims_create_button($_DIMS['cste']['_DIMS_VALID'],"./common/img/publish.png","javascript:document.location.href='$scriptenv?cat="._BUSINESS_CAT_CONTACT."&op=validpublicshare&action="._BUSINESS_TAB_ENT_FORM."&id_tiers=".$ent->fields['id']."&part="._BUSINESS_TAB_ENT_IDENTITE."';","","");
							echo "</td>";
						}
					}
					else {
						...
						if (isset($_SESSION['dims']['tiers_fields_view'][$fields['id']])) {
							switch($_SESSION['dims']['tiers_fields_view'][$fields['id']]) {
								case 0:
									if ($entworkspace->fields[$fields['namefield']]!='' && $replies[$fields['id']][0]!=$entworkspace->fields[$fields['namefield']]) {
											$elem= array();
											$elem['icon']='users.png';
											$elem['label']=$_DIMS['cste']['_WORKSPACE'];
											$elem['value']=$entworkspace->fields[$fields['namefield']];
											$elem['mode']=1;
											$elem['id']=$entworkspace->fields['id_layer'];
											$arrayOtherValues[]=$elem;
									}
									elseif ($entuser->fields[$fields['namefield']]!='' && $replies[$fields['id']][0]!=$entuser->fields[$fields['namefield']]) {
											$elem= array();
											$elem['icon']='user.png';
											$elem['label']=$_DIMS['cste']['_DIMS_LABEL_USER'];
											$elem['value']=$entuser->fields[$fields['namefield']];
											$elem['mode']=2;
											$elem['id']=$entuser->fields['id_layer'];
											$arrayOtherValues[]=$elem;
									}
									break;
								...
								case 1:
									...
									if ($entuser->fields[$fields['namefield']]!='' && $replies[$fields['id']][0]!=$entuser->fields[$fields['namefield']]) {
											$elem= array();
											$elem['icon']='user.png';
											$elem['label']=$_DIMS['cste']['_DIMS_LABEL_USER'];
											$elem['value']=$entuser->fields[$fields['namefield']];
									...
									elseif ($ent->fields[$fields['namefield']]!='' && $replies[$fields['id']][0]!=$ent->fields[$fields['namefield']]) {
											if ($_SESSION['dims']['tiers_fields_mode'][$fields['id']]==0) {
													$icon_field='all.png';
													$label=$_DIMS['cste']['_DIMS_LABEL_PUBLIC'];
											}
											else {
											...
											$elem['label']=$label;
											$elem['value']=$ent->fields[$fields['namefield']];
											$elem['mode']=0;
											$elem['id']=0;
											$arrayOtherValues[]=$elem;
									}
					...
					// construction du tableau de valeurs diff�rentes et/ou partag�es
					if (!empty($arrayOtherValues)) {
						// ligne sup
...
<script language="javascript">
	...
	echo "function validate_dyn_field() {";
		...
		/*dims_validatefield('<? echo $tabfiltrefield[0]['label']; ?>',document.getElementById("field<? echo $tabfiltrefield[0]['id']; ?>"), 'string')
		*/
// on genere le test
/*
 * dims_validatefield('<? echo $tabfiltrefield[0]['label']; ?>',document.getElementById("field<? echo $tabfiltrefield[0]['id']; ?>"), 'string') &&
			dims_validatefield('<? echo $tabfiltrefield[1]['label']; ?>',document.getElementById("field<? echo $tabfiltrefield[1]['id']; ?>"), 'string')
 */
			...
			foreach ($arrayfield_control as $ind=>$f) {
				...
				$name=str_replace("'"," ",$name);
					...
					if ($f['type']=='checkbox' || $f['type']=='radio') {
						...
						echo "dims_validatefield('".$name."','field".$f['id']."[]', '".$f['type']."')";
					}
					else {
						echo "dims_validatefield('".$name."',document.getElementById('field".$f['id']."'), '".$f['type']."')";
					}
			}

...
if (!empty($array_modules)) {
		...
		if ($block['label']!="system") {
			...
			if (file_exists("./common/modules/".$block['label']."/block_portal.php")) {
				echo "<tr><td width=\"28%\">
					...
					<tr><td colspan=\"2\">
						<div style=\"padding-left:4px;width:100%;$display\" id=\"content".$block['instanceid']."\"></div>
					</td></tr>
				";
			}
		}
	...
	if(isset($_SESSION['dims']['active_search'][$_SESSION['dims']['workspaceid']]) && $_SESSION['dims']['active_search'][$_SESSION['dims']['workspaceid']])
		echo "<SCRIPT LANGUAGE=\"javascript\">searchWord();</script>";
	else
		echo "<SCRIPT LANGUAGE=\"javascript\">searchRecursiveNews();</script>";
}

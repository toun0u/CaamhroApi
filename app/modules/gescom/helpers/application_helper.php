...
namespace Gescom {
	...
	function get_path($params = array()){
		return \dims::getInstance()->getScriptEnv()."?".http_build_query($params);
	}
	...
	*/
	...
	function getManagers() {
		...
		$res = $db->query($sel, array(
			// FIXME : Avoid session - Should use dims get to get workspaceid
			':idw' => $_SESSION['dims']['workspaceid'],
		));

		while($manager = $db->fetchrow($res)){
			$managerslist[$manager['id']] = $manager['firstname'].' '.$manager['lastname'];
		}

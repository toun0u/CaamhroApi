            ...
            <div id="slidesContainer_{$slider.id}" class="slidesContainer">
                {foreach from=$articles item=art_slide name=art_slide}
                    <div class="slide_{$slider.id} slide">
                        {include file='slide_accueil.tpl' article=$art_slide}
                    </div>
                {/foreach}
            </div>
        </div>
...
<script type="text/javascript">
    ...
    jQuery('#slideshow_{$slider.id}')
      .prepend('<span class="control_{$slider.id} control_slide left" id="leftControl_{$slider.id}" style="background-color:#0060B0"></span>')
      .append('<span class="control_{$slider.id} control_slide right" id="rightControl_{$slider.id}" style="background-color:#0060B0"></span>');

  ...
  foreach ($this->getlignes() as $ligne) {
	...
	if ($oCatalogue->getParams('cata_base_ttc')) {
	  ...
	  $message .= "
		...
		<TD> {$ligne['ref']} </TD>
		<TD> {$ligne['label']} </TD>
		<TD ALIGN=\"RIGHT\"> ".catalogue_formateprix($ligne['pu_ht'])." &euro; </TD>
		<TD ALIGN=\"RIGHT\"> {$ligne['qte']} </TD>
		<TD ALIGN=\"RIGHT\"> ".catalogue_formateprix(sprintf("%.2f",round($ligne['qte'] * $ligne['pu_ht'],2)))." &euro; </TD>
		</TR>";
	}
...
// if ($oCatalogue->getParams('cata_base_ttc')) {
  ...
  $message .= "
	<TR CLASS=\"ligne1\">
	  ...
	  <TD WIDTH=\"100px\" ALIGN=\"RIGHT\"> <B>".catalogue_formateprix($this->fields['total_ht']-$this->fields['port'])." &euro;</B> </TD>
	</TR>
	<TR CLASS=\"ligne1\">
	  <TD> Port </TD>

...
// $rubriques = dims_load_securvalue('param', dims_const::_DIMS_NUM_INPUT, true, true, true);
$rubriques = dims_load_securvalue('rubriques', dims_const::_DIMS_NUM_INPUT, true, true, true);
...
if (!isset($_SESSION['catalogue']['pagination']['articles']['catalogue']['nbElems'])) {
	$_SESSION['catalogue']['pagination']['articles'][$motscles.$rubriques]['nbElems'] = $a_pagination_per_page[1];
	$_SESSION['catalogue']['pagination']['articles'][$motscles.$rubriques]['page'] = 0;
}
...
// onglets
$famille_menu = 0;
if (isset($familys['list'][$rubriques])) {
...
if ( $oCatalogue->getParams('cata_nav_style') == 'arbo' && !empty($familys['tree'][$rubriques]) ) {
	...
	$a_goIds = article::simple_search($motscles, $catalogue_moduleid);
	...
	$sql = "SELECT a.`id`, af.`id_famille`
		...
		AND 		a.`id_globalobject` IN (".implode(',', $a_goIds).")";
	...
	// // Filtrage en fonction du dépot auquel est rattaché le client
	// if (!empty($_SESSION['catalogue']['id_company'])) {
	// 	$sql .= '
...
//die('r');
	...
	while ($fields = $db->fetchrow($rs)) {
		...
		if ($prix > 0 || $condition_connected_OR_notBTOC) {
			...
			if(empty($a_famFiltre) || in_array($fields['id_famille'],$a_famFiltre)){
		...
		// Pour l'affichage de l'arborescence
		if (!is_null($fields['id_famille'])) {
			if (!isset($a_artFam[$fields['id_famille']])) $a_artFam[$fields['id_famille']] = 0;
			$a_artFam[$fields['id_famille']]++;
		}
			...
			switch ($familys['list'][$idParent]['depth']) {
				case 2:
					$catalogue['fam1'][$idParent] = array(
						...
						'lien'		=> '?op=recherche&motscles='.$motscles.'&rubriques='.$idParent,
...
$_SESSION['dims']['tpl_page']['TITLE'] = 'Recherche de produits';
$_SESSION['dims']['tpl_page']['META_DESCRIPTION'] = 'Rechercher des produits par mots-clés';
$_SESSION['dims']['tpl_page']['META_KEYWORDS'] = 'recherche, '.$motscles;

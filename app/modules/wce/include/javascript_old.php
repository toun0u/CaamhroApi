...
if ($_SESSION['dims']['action'] == 'admin') {
	...
	var boxedit=null;
        ...
        function updateStateWce(object,type,id,value) {
	...
	function moveScroller() {
		...
		.dialog({
			...
			position:['right',0]
             ...
             //var dopen = $("#dims_editbox").dialog({ autoOpen: false });
            ...
            $("div.activedimsbloc").hover(
                function(event) {
                    ...
                    /*var new_elem=$("#dims_selectedelement");//.clone().attr('id','dims_selectedelement');
                    var position=$(this).offset();
                    new_elem.css('left',position.left);
                    new_elem.css('top',position.top);

                    new_elem.css('width',$(this).width());
                    new_elem.css('height',$(this).height());
                    new_elem.css('display', 'block');
                    set_opacity('dims_selectedelement','30');
                    //new_elem.css('z-index',950);
                    new_elem.css('visibility', 'visible');
		...
		//var Pos   = ObjGetPosition( id_bloc);
		if (!$editbox.dialog( "isOpen" )) {
		//if (boxedit==null){
			boxedit=$editbox.dialog('open');
			Obj=boxedit;//$("#dims_editbox");
			var cal = $("#dims_editbox");//$('#' + $(this).data('colorpickerId'));
		}
		else {
        ...
        function initCurrentEditObject(id_bloc) {
            var select="";
	...
	function wceAddBlock(section) {
        if( arguments[0] == null) section='';

		dims_showcenteredpopup("",700,'','dims_popup');
		dims_xmlhttprequest_todiv('admin.php','dims_op=add_block&section='+section,'','dims_popup');
	}

	function wceModifBlock(blockid) {
		dims_showcenteredpopup("",700,'','dims_popup');
		dims_xmlhttprequest_todiv('admin.php','dims_op=modify_block&block_id='+blockid,'','dims_popup');
	}

	function wceModifDescription(articleid) {
		dims_showcenteredpopup("",700,550,'dims_popup');
		dims_xmlhttprequest_todiv('admin.php','op=modify_desc_article&article_id='+articleid,'','dims_popup');
	}

	function wceMoveBlockContent(sens,blockid,contentid) {
	...
	function wceModifBlockContent(blockid,contentid) {
		dims_showcenteredpopup("",900,550,'dims_popup');
		dims_xmlhttprequest_todiv('admin.php','dims_op=modify_blockcontent&block_id='+blockid+'&content_id='+contentid,'','dims_popup');
	}


	function activeWceDescription (articleid) {
	...
	function activeWceButton(active) {
			if (active) {
				...
				if (document.getElementById('publish1')!=null) {
	...
	function wce_refreshedit_article() {
		if (currentmode=="edit") {
			if (document.getElementById('edit')!=null) {
				document.getElementById('edit').style.visibility='visible';
				document.getElementById('edit').style.display='';
	...
	function wce_getcontent_article(articleid,wce_mode,headingid) {
		var version=0;
	...
	function wce_showheading(hid,str) {
		...
		if (elt = document.getElementById(hid)) {
			if (elt.style.display == 'none') {
				if (elt.innerHTML.length < 20) dims_xmlhttprequest_todiv('<? echo $scriptenv; ?>','op=xml_detail_heading&hid='+hid+'&str='+str,'',hid);
	...
	function wce_heading_validate(form) {
	...
	function wce_article_validate(form, article_type, article_status, validator) {
		next = true;
		valid=false;
	...
	//-----------------------
	function DIV_InitScroll(namediv){
	  //-- Recup position Objet
	...
	function checkAllArticles(nbfiles) {
		for (i = 0; i < nbfiles; i++)
			document.getElementById("selart"+i).checked = true;
	}

	function uncheckAllArticles(nbfiles) {
		for (i = 0; i < nbfiles; i++)
			document.getElementById("selart"+i).checked = false;
	}

	function validCommandArt(event,currentheading,nbfiles) {
		var cpte=0;
		var selectheadings="";
	...
	function displayHeadingChoice(event,currentheading,selectheadings) {

...
$indicesection=1;
$section_pagination = new wce_section_pagination($this->fields['id'].'_'.$section['id']);
$nblevel=wce_article::NB_LEVEL;

// valeur courante
// $_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$id]

$PageBreak=$this->GetPageBreakBlocks();
$ispageBreak=(isset($PageBreak[$section['id']])) ? $PageBreak[$section['id']] : 0;

if ($ispageBreak) {
    $currentpagesection=dims_load_securvalue('WCE_section_'.$this->fields['id'].'_'.$section['id'],dims_const::_DIMS_NUM_INPUT,true,true,false,$_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']],$_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']]);

    // page break par defaut le premier si non defini
    if (!isset($_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']]) || $_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']]<=0) {
        $currentpagesection=1;
        $_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']]=1;
    }
    else
        $currentpagesection=$_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']];
}
else {
    // pas de page break;
        ...
        if ($currentpagesection==0 || $currentpagesection==$indicesection ) {
            ...
            if (isset($models[$block['id_model']])) {
                ...
                if ($levelblock<=$nblevel && $block['position']>1 && $diffpos<1) {
                    ...
                    $contentsection.='<span style="float:right;margin-left:5px;"><img src="'.module_wce::getTemplateWebPath('/gfx/arrow-right-dis.png').'" border="0"></span>';
                }
                ...
                if ($levelblock>1) {
                    ...
                    $contentsection.='<span style="float:right;margin-left:5px;"><img src="'.module_wce::getTemplateWebPath('/gfx/arrow-left-dis.png').'" border="0"></span>';
                }
                ...
                if ($block['position']>1) {
                    ...
                    $contentsection.='<span style="float:right;margin-left:5px;"><img src="'.module_wce::getTemplateWebPath('/gfx/arrow-up-dis.png').'" border="0"></span>';
                }
                ...
                if ($block['position']<sizeof($blocks)) {
                    ...
                    $contentsection.='<span style="float:right;margin-left:5px;"><img src="'.module_wce::getTemplateWebPath('/gfx/arrow-bottom-dis.png').'" border="0"></span>';
                }
                ...
                for($i=1;$i<=10;$i++) {
						...
						if (isset($block['contentversion'])) {
							...
							if ($ischange) {
								...
								$contblock=preg_replace('/ href=["|\'][a-zA-Z-_\/.0-9:+?%=&;,]*articleid=([0-9]+)[a-zA-Z-_\/.0-9:+?%=&;,]*((&|&amp;)WCE_section_[0-9]+_[0-9]+=[0-9]+#[0-9]+)?["|\']/i'," href=\"#\" onclick=\"javascript:window.parent.updateCompleteArticle('".module_wce::get_url($_SESSION['dims']['wce']['sub'])."&sub=".module_wce::_SITE_PREVIEW."&action=".module_wce::_PREVIEW_EDIT."&articleid=$1$2');\" ",$block['content'.$i]);
...
if ($currentpagesection>$section_pagination->getMaxItem()) {
    ...
    $_SESSION['dims']['wcecurrentsections'][$this->fields['id']][$section['id']]=$currentpagesection;
}
...
$contentsection.='</div>';
$contentsection.='<script type="text/javascript"> window[\'saveBloc\'] = function saveBloc(id,idArt,ico,label){

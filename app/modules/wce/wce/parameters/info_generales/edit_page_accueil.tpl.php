<div class="title_h3">
    <h3>Pages d'accueil : <? echo $this->fields['domain']; ?></h3>
</div>
<div class="form_object_block">
	<form method="POST" action="<? echo module_wce::get_url(module_wce::_SUB_PARAM); ?>" name="save_meta">
		<input type="hidden" name="action" value="<? echo module_wce::_PARAM_INFOS_SAVE_ACCUEIL; ?>" />
				...
				<table>
					<tr>
						<td class="title_table">
							...
							$lk = "";
							if ( $this->getDefaultHomePage() != '' && $this->getDefaultHomePage() > 0) {
								$home->open($this->getDefaultHomePage());
								$lk = "&articleid=".$this->getDefaultHomePage();
							}
							?>
							<input type="hidden" id="wce_article_id_article_link_HOME" name="wce_article_id_article_link_HOME" value="<? echo $home->fields['id']; ?>">
							<input style="float:left;width:40%;" type="text" readonly class="text" id="linkedpage_displayed_HOME" value="<? echo $home->fields['title']; ?>">
						</td>
						<td>
							<input type="button" style="width:auto;" class="button" value="<? echo $_SESSION['cste']['_FORM_SELECTION']; ?>" onclick="javascript:dims_showpopup('',300,event,'click','dims_popup');dims_xmlhttprequest_todiv('admin-light.php','dims_op=selectlinkarticle&input=wce_article_id_article_link_HOME&display=linkedpage_displayed_HOME<? echo $lk; ?>',false,'dims_popup');"/>&nbsp;
							<input type="button" style="width:auto;" class="button" value="<? echo $_SESSION['cste']['_DIRECTORY_LEGEND_DELETE']; ?>" onclick="javascript:dims_getelem('wce_article_id_article_link_HOME').value='';dims_getelem('linkedpage_displayed_HOME').value='';" /></span>
						</td>
					</tr>
				</table>
			</div>

...
<table class="table_referencement" cellspacing="0" cellpadding="0" border="1" style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
	...
	foreach($domaines as $r){
		...
		<tr <? echo $trl; ?>>
			...
			<td>
				<?
				if ($r['id_home_wce_article'] != '' && $r['id_home_wce_article'] > 0){
					$art = new wce_article();
					$art->open($r['id_home_wce_article']);
					?>
			...
			<td class="actions">
				<a href="<? echo module_wce::get_url(module_wce::_SUB_PARAM)."&sub=".module_wce::_PARAM_INFOS."&action=".module_wce::_PARAM_INFOS_EDIT_ACCUEIL."&id=".$r['id']; ?>">

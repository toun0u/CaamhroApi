...
$urlpost=$dims->getUrlPath().'?op=event&action=save_niv1&id_event='.$id_action.'&workspace_code='.$workspace_code;
$urlget=$dims->getUrlPath().'?op=event&action=form_niv1&id_event='.$id_action.'&workspace_code='.$workspace_code;
//echo $urlpost;die();

?>
<div>
	<form action="<?php echo $urlget; ?>" method="POST" id='form_multi_inscrip'>
		<p>
...
<div id="form_1">
	<!--h2>
		...
		//$urlpost=dims_urlencode($dims->getUrlPath().'?action=save_niv1&id_event='.$id_action.'&workspace_code='.$workspace_code);
		$urlpost='/index.php?action=save_niv1&id_event='.$id_action.'&workspace_code='.$workspace_code;
		//echo $urlpost;die();
		?>
	</h2-->
	<form action="<? echo $urlpost; ?>" method="POST" id='form_inscrip_niv1'>
		...
		<div class="inscriptions">
			...
			<div class="captcha">
				<p>
					...
					<img src="./common/modules/events/cms_captcha.php?<?= mktime(); ?>" /><br />
			...
			<?php
				...
				if($evt->fields['timestp_open'] <= date('YmdHis')) {
					echo '<input type="submit" value="'.dims_constant::getVal('_SUBMIT').' >" class="submit" />';
				}
				else {

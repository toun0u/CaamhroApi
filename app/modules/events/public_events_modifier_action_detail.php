...
<form name="form_action" action="admin.php?dims_mainmenu=events&dims_desktop=block&dims_action=public" method="post">
	<input type="hidden" name="submenu" value="<?php echo dims_const::_DIMS_SUBMENU_EVENT; ?>">
	...
	<div id="block_content0" style="float:left;width:99%;">
	<?php
		echo "<h2 class=\"title_description\" style=\"padding:2px;margin-top:10px;\"><img src=\"./common/img/administration.png\" alt=\"event\">&nbsp;".$_DIMS['cste']['_DIMS_PROPERTIES']."</h2>";
	//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PROPERTIES']);
			...
			<td width="50%">
				<table cellpadding="0" cellspacing="5" width="100%">
										...
										foreach($listenum as $tab_id => $tabe) {
												...
												if($id_tab == false ) {
														unset($listenum[$tab_id]);
																...
																$action->fields['typeaction'] = dims_load_securvalue('type_action',dims_const::_DIMS_CHAR_INPUT,true,false,true,$action->fields['typeaction']);

								foreach($listenum as $id_enum => $enum) {
										if(isset($action->fields['typeaction']) && $enum['libelle'] == $action->fields['typeaction']) $sel = 'selected';
					...
					<tr>
						...
						<td align="left">
													<?
													if ($id==0 && $type==dims_const::_PLANNING_ACTION_EVT) $action->fields['libelle']=$_DIMS['cste']['_RSS_LABELTAB_ADD']." ".$_DIMS['cste']['_DIMS_LABEL_EVENT'];
													?>
	...
	require_once DIMS_APP_PATH . '/modules/events/public_events_modifier_action_detail_dates.php';

	//echo $skin->open_simplebloc($_DIMS['cste']['_WCE_ARTICLE_PUBLISH']);
		echo "<h2 class=\"title_description\" style=\"padding:2px;margin-top:10px;\"><img src=\"./common/img/suivi_suivant.png\" alt=\"event\">&nbsp;".$_DIMS['cste']['_WCE_ARTICLE_PUBLISH']."</h2>";
	echo '<table cellpadding="0" cellspacing="5" width="100%">';
	?>
		<tr>
			<td width="50%" valign="top">
				<table cellpadding="0" cellspacing="5" width="100%">
					...
					<tr>
						...
						<td width="80%">
							...
							<tr>
								...
								<td align="left">
									<input type="radio" name="supportrelease" id="supportrelease_no" value="0" <?php echo ($action->fields['supportrelease'] !=1) ? 'checked' : ''; ?>>
								</td>
							</tr>
							</table>
						</td>
					...
					<?php
										...
										if (!$action->new && $action->fields['typeaction'] == '_DIMS_PLANNING_FAIR_STEPS') {
					?>
					<tr>
						<td align="right" width="20%">
					...
					<?php
						...
						//Dans le cas d'une foire, le prix est affiché directement dans la page
						...
						<td colspan="2">
							...
							<div id="evtMoreOptions" style="display: none;">
								<table width="100%">
									...
									<tr>
										<td align="right">
											<label for="evt_labelfield_<?php echo $ii; ?>"><?php echo $_DIMS['cste']['_DIMS_LABEL_LABEL']." ".$ii; ?>&nbsp;</label>
										</td>
										<td width="50%">
											<input name="evt_labelfield_<?php echo $ii; ?>"  style="width:200px" type="text" class="text" value="<?php echo $action->fields['labelfield_'.$ii]; ?>"
										</td>
									</tr>
									<?
									}
			...
			<td  valign="top">
				<table cellpadding="0" cellspacing="5" width="100%" >
					<tr>
						...
						// intégration des éléments de l'event
		...
		if($type == dims_const::_PLANNING_ACTION_RDV)
			echo dims_create_button($_DIMS['cste']['_DIMS_SAVE'],"./common/img/save.gif","javascript:if (!business_verif_action(document.form_action)) {alert('Vous devez sÃ©lectionner un client et un dossier');return(false)}","enreg","width:150px");
		elseif($id != 0)
		...
		//if($id==0 || $id == ""){
			echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(1);','','width:150px;float:left;');
		//}
		...
		//	if($type == dims_const::_PLANNING_ACTION_EVT) {
		//		echo dims_create_button($_DIMS['cste']['_DELETE'],'./common/img/del.png',"javascript:document.form_action.action.value='xml_planning_action_supprimer';dims_confirmform(document.form_action,'Etes-vous certain ?');",'delete1','width:100px');
		//	}
		//}
		echo '</div>';
		?>
	...
	<div id="block_content1" style="display:none;width:100%;float:left;overflow:auto;">
		<table cellpadding="0" cellspacing="5" width="100%">
			<tr>
				<td>
					<table cellpadding="0" cellspacing="5" width="100%">
						...
						<tr>
							...
							<td >
								<input type="text" name="fck_evt_lieu" id="fck_evt_lieu" class="text" value="<?php echo $action->fields['lieu']; ?>" />
												...
												<tr>
							<td align="right" width="100px">
								<label for="evt_lieu"><?php echo $_DIMS['cste']['_DIMS_LABEL_COUNTRY']; ?>&nbsp;</label>
							</td>
							<td>

															<select name="evt_id_country">
															...
															if ($db->numrows($resu)>0) {
																while ($a=$db->fetchrow($resu)) {
																	if (isset($action->fields['id_country']) && $action->fields['id_country']==$a['id']) $selectc='selected';
																	else $selectc='';

																	echo "<option value=\"".$a['id']."\" ".$selectc.">".$a['printable_name']."</option>";
																}
															}
															?>
															</select>
				...
				<td>
					...
					<table cellpadding="0" cellspacing="5" width="100%">
						<tr>
							...
							<td width="80%">
								<textarea style="width: 400px;" name="fck_action_teaser" id="fck_action_teaser" rows="5" cols="30" class="text"><?php echo html_entity_decode($action->fields['teaser']); ?></textarea>
							</td>
						</tr>
					</table>
					<?php
			...
			<tr>
				<td colspan="2">
					<table cellpadding="0" cellspacing="5" width="100%">
						<td align="right" width="100px" valign="top"><?php echo $_DIMS['cste']['_DIMS_LABEL_DESCRIPTION']; ?></td>
						<td align="left">
							<?php
			...
			<?php
				...
				if($id > 0) {
					if($type == dims_const::_PLANNING_ACTION_RCT) {
						echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(3);','','width:150px;float:left;');
					} elseif($action->fields['typeaction'] == '_DIMS_PLANNING_FAIR_STEPS') {
						echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(5);','','width:150px;float:left;');
					}
					elseif($action->fields['typeaction'] == '_DIMS_PLANNING_FAIR' || $enabledAdminEvent) {
						echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(3);','','width:150px;float:left;');
					}
					else {
						echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(2);','','width:150px;float:left;');
					}
				}
	...
	<div id="block_content2" style="display:none;width:99%;">
		<?php
		if($type == dims_const::_PLANNING_ACTION_RDV) {
			...
			// traitement par dÃ©faut des user sÃ©lectionnÃ©s =>
			// si action existante il faut remonter les donnÃ©es
			if ($action->fields['id']>0) {
				$res=$db->query("SELECT user_id,participate
								FROM dims_mod_business_action_utilisateur
								WHERE action_id= :idaction
								AND user_id in (".implode(",",$_SESSION['dims']['planning']['currentactionusers']).")",
								array(':idaction' => $action->fields['id'])
								);
			...
			$arraylist=$dims_user->getusersgroup($nomsearch,$_SESSION['dims']['planning']['currentworkspacesearch'],$_SESSION['dims']['planning']['currentprojectsearch']);
			...
			// on affiche par dÃ©faut les personnes sÃ©lectionnÃ©es et prÃ©select
			if (!empty($arraylist)) {
				$res=$db->query("select u.* from dims_user as u where id in (".implode(",",$arraylist).")");
				if ($db->numrows($res)>0) {
			...
			if (!empty($lstusers)) {
				$res=$db->query("select u.* from dims_user as u where id in (".implode(",",$lstusers).")");
				if ($db->numrows($res)>0) {
					echo "<table style=\"width:100%;\">";
					while ($f=$db->fetchrow($res)) {
						//calcul de l'icon
		...
		elseif($type == dims_const::_PLANNING_ACTION_EVT) {
			...
			// traitement par dÃ©faut des partenaire sÃ©lectionnÃ©s =>
			// si action existante il faut remonter les donnÃ©es
			if ($action->fields['id']>0 && !empty($_SESSION['dims']['planning']['currentactionpartner']))
			{
				$res=$db->query("SELECT tiers_id,participate
								FROM dims_mod_business_action_detail
								WHERE action_id= :idaction
								AND tiers_id in (".implode(",",$_SESSION['dims']['planning']['currentactionpartner']).")",
								array(':idaction' => $action->fields['id'])
								);
			...
			//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_LABEL_PARTNER']);
			...
			<div id="lst_planningtemppartner" style="width:100%;overflow:auto;height:105px;display:block;float:left;"></div>
			<?php
			echo "</div>";
			...
			if (!empty($lstusers)) {
				$res=$db->query("select t.* from dims_mod_business_tiers as t where id in (".implode(",",$lstusers).")");
				if ($db->numrows($res)>0) {
					echo "<table style=\"width:100%;\">";
					while ($f=$db->fetchrow($res)) {
						//calcul de l'icon
		...
		elseif($type == dims_const::_PLANNING_ACTION_RCT) {
			...
			{
				$res=$db->query("SELECT contact_id,participate
								FROM dims_mod_business_action_detail
								WHERE action_id= :idaction
								AND contact_id in (".implode(",",$_SESSION['dims']['planning']['currentactionusers']).")",
								array(':idaction' => $action->fields['id'])
								);
			...
			if (!empty($_SESSION['dims']['planning']['currentactionusers'])) $lstusers+=$_SESSION['dims']['planning']['currentactionusers'];
			if (!empty($lstusers)) {
				$res=$db->query("select c.* from dims_mod_business_contact as c where id in (".implode(",",$lstusers).")");
				if ($db->numrows($res)>0) {
					echo "<table style=\"width:100%;\">";
					while ($f=$db->fetchrow($res)) {
						//calcul de l'icon
...
<script language="javascript">
	...
	function execSearchLink() {
		...
		if(nomsearch.length>=2) {
			dims_xmlhttprequest_todiv("admin.php", "action=search_ct&search="+nomsearch+"&id=<? echo $action->fields['id']; ?>", "", 'searchArea' );
			divtoaffich.style.display = "block";
		}
	}
</script>
	<div id="block_content3" style="float:left;display:none;width:99%;">
				<?php
				/**** Affichage des inscriptions ****/
				//echo '</td><td valign="top">';
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_EVT_CONTACT_REGISTER']);
								echo "<h2 class=\"title_description\" style=\"padding:2px;margin-top:10px;\"><img src=\"./common/img/suivi_suivant.png\" alt=\"event\">&nbsp;".$_DIMS['cste']['_DIMS_EVT_CONTACT_REGISTER']."</h2>";
				...
				$ress = $db->query($sql,array(':idaction' => $_SESSION['dims']['currentaction']));
		...
		echo dims_create_button($_DIMS['cste']['_DIMS_SAVE'],"./common/img/save.gif","javascript:form_action.submit();","enreg","width:150px;float:left;");
		echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_NEXT_MILESTONE'],'./common/img/go-next.png','javascript:change_menu(6);','','width:150px;float:left;');
		echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_CANCEL'],'./common/img/delete.png',"location.href='admin.php'",'cancel1','width:90px;float:right;');
		echo '</div>';
	?>
	</div>
...
<div id="block_content5" style="display:none;width:100%;float:left;overflow:auto;">
	<?php

	if($typeaction == '_DIMS_PLANNING_FAIR_STEPS' && $action->fields['niveau'] == 2 && $action->fields['is_model']==0) {
		//on propose l'application d'un modèle
		//on sélectionne les modèles disponibles
		$sql_m = 	"SELECT id, libelle
					FROM dims_mod_business_action
					WHERE typeaction='_DIMS_PLANNING_FAIR_STEPS'
					AND is_model >= 1
					AND id_workspace = :idworkspace
					AND libelle LIKE '%_model'";

		$res_m = $db->query($sql_m, array(':idworkspace' => $_SESSION['dims']['workspaceid']) );
		if($db->numrows($res_m) > 0) {
			...
			<tr>
				...
				<td align="left">
					<select class="select" name="action_is_model" id="action_is_model" onchange="javascript:document.form_action.submit();">
						<option value="0">--</option>
			<?
	...
	echo "</form>";
		if(!$action->new && $niveau == 2 ) //xml_planning_modifier_action
			require_once(DIMS_APP_PATH . '/modules/events/public_events_etape.php');
...
<div id="block_content6" style="float:left;display:none;width:99%;">
	<?php
		if($type == dims_const::_PLANNING_ACTION_EVT) {
			...
			?>
			<form name='ress_front' id='ress_front' action='admin.php?action=xml_planning_modifier_action&id=<?php echo $action->fields['id']; ?>&subtab=6' method='post' enctype="multipart/form-data" style="margin-left: 20px;">
			<input type="hidden" name="MAX_FILE_SIZE" value="20097152" />
				<?
								echo "<h2 class=\"title_description\" style=\"padding:2px;margin-top:10px;\"><img src=\"./common/img/suivi_suivant.png\" alt=\"event\">&nbsp;".$_DIMS['cste']['_DIMS_LABEL_BANNER']."</h2>";
				...
				<p>
					...
					?>
						<input type="file" name="banner" id="banner" />&nbsp;
							<input type="button" value="<?php echo $_DIMS['cste']['_DIMS_SEND']; ?>" onclick="javascript:document.ress_front.submit();"/>
					<?php
						}
					?>
				</p>
				...
				<p>
					...
					if ($action->fields['id']>0)
					{
						...
						// collecte des fichiers deja insÃ©rÃ©s
				...
				<?	echo $skin->close_simplebloc();
					 */
				echo "<h2 class=\"title_description\" style=\"padding:2px;margin-top:10px;\"><img src=\"./common/img/suivi_suivant.png\" alt=\"event\">&nbsp;".$_DIMS['cste']['_DIMS_LABEL_EVENT_MATCHMAKING']."</h2>";
		...
		echo dims_create_button($_DIMS['cste']['_DIMS_BACK'],'./common/img/go-previous.png','javascript:change_menu(3);','','width:90px;float:left;');
		echo dims_create_button($_DIMS['cste']['_DIMS_SAVE'],"./common/img/save.gif","javascript:ress_front.submit();","enreg","width:100px;float:left;");
		echo dims_create_button($_DIMS['cste']['_DIMS_LABEL_CANCEL'],'./common/img/delete.png',"location.href='admin.php'",'cancel1','width:90px;float:right;');
		echo '</div>';
	?>
</div>
<div id="block_content7" style="float:left;display:none;width:99%;">

</div>

<div style="float:left; width:100 %; clear: both; text-align: right;">
	<?php
	if(isset($action->fields['id']) && !empty($action->fields['id']) && ($action->fields['id_user']==$_SESSION['dims']['userid'] || dims_isadmin())){
			if($type != dims_const::_PLANNING_ACTION_EVT) {
				echo dims_create_button($_DIMS['cste']['_DELETE'],'./common/img/del.png',"javascript:document.form_action.action.value='xml_planning_action_supprimer';dims_confirmform(document.form_action,'Etes-vous certain ?');",'delete1','width:100px');

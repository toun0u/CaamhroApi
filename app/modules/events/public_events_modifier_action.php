...
$subaction=dims_load_securvalue('subaction',dims_const::_DIMS_CHAR_INPUT,true,true,false,$_SESSION['dims']['subaction'],$_SESSION['dims']['subaction']);

$action = new action();
// chargement de la valeur courante et allocation eventuelle si passage de l'id
$id		= dims_load_securvalue('id',dims_const::_DIMS_NUM_INPUT,true,false,true,$_SESSION['dims']['currentaction']);
$type	= dims_load_securvalue('type',dims_const::_DIMS_NUM_INPUT,true,false,true,$type,_DIMS_ACTION_MAIN);
...
if ($id>0) {
		switch($action->fields['type']) {
			case dims_const::_PLANNING_ACTION_RDV :
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_MODIFY_RDV']." : ".$action->fields['libelle']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_MODIFY_RDV']." : ".$action->fields['libelle'];
			break;

			case dims_const::_PLANNING_ACTION_EVT :
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_MODIFY_EVT']." : ".$action->fields['libelle']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_MODIFY_EVT']." : ".$action->fields['libelle'];
			break;

			case dims_const::_PLANNING_ACTION_RCT :
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_MODIFY_RCT']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_MODIFY_RCT'];
			break;
		};
} else {
		switch($type) {
			case dims_const::_PLANNING_ACTION_RDV :
			   // echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_ADD_RDV']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_ADD_RDV'];
			break;

			case dims_const::_PLANNING_ACTION_EVT :
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_ADD_EVT']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_ADD_EVT'];
			break;

			case dims_const::_PLANNING_ACTION_RCT :
				//echo $skin->open_simplebloc($_DIMS['cste']['_DIMS_PLANNING_ADD_RCT']);
				$title_bloc = $_DIMS['cste']['_DIMS_PLANNING_ADD_RCT'];
			break;
		};

		$datejour=dims_load_securvalue("datejour",dims_const::_DIMS_CHAR_INPUT,true);
...
echo "<h2 style=\"font-size:16px;padding:2px;\"><img src=\"./common/img/event.png\" alt=\"event\">&nbsp;".$title_bloc."</h2>";
?>
...
<script language="javascript" type="text/javascript">
	...
	function searchUserActionPlanningExec() {
		...
		dims_xmlhttprequest_todiv('admin.php','action=search_action_contact_planning&nomsearch='+nomsearch,'||',"lst_planningtempuser","selectedusers");
	}
	...
	function searchPartnerActionPlanningExec() {
		...
		dims_xmlhttprequest_todiv('admin.php','action=search_action_partner_planning&nomsearch='+partsearch,'||',"lst_planningtemppartner","selectedpartner");
	}
	...
	function delete_doc(id_doc) {
		dims_xmlhttprequest('admin.php','action=delete_fileannonce&id_doc='+id_doc);
		document.form_action.submit();
	}

	function delete_banner(id_evt) {
		dims_xmlhttprequest('admin.php','action=delete_banner&id_evt='+id_evt);
		document.form_action.submit();
	}

	function delete_match(id_evt) {
		dims_xmlhttprequest('admin.php','action=delete_match&id_evt='+id_evt);
		document.form_action.submit();
	}

	function delete_preview(id_evt) {
		dims_xmlhttprequest('admin.php','action=delete_preview&id_evt='+id_evt);
		document.form_action.submit();
	}
...
function change_menu(num){
	...
	dims_xmlhttprequest('admin.php', 'action=update_actionetap&step='+num);
}

function deleteEtap(id_actionetap) {
	dims_xmlhttprequest('admin.php', 'dims_mainmenu=events&dims_desktop=block&dims_action=public&action=delete_actionetap&id_actionetap='+id_actionetap);
	//dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';
}

function deleteAllEtapes(id) {
	dims_xmlhttprequest('admin.php', 'dims_mainmenu=events&dims_desktop=block&dims_action=public&action=delete_actionalletapes&id=<?php echo $_SESSION['dims']['currentaction']; ?>');
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';
}

function deleteFile(id_etapFile) {
	dims_xmlhttprequest('admin.php', 'dims_mainmenu=events&dims_desktop=block&dims_action=public&action=delete_fileetap&id_fileetap='+id_etapFile);
	//dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';
}
...
function addEtap(id_etap) {
	...
	dims_xmlhttprequest_post('admin.php', 'action=save_actionetap&actionetap_description='+text+'&actionetap_label='+label+'&actionetap_condition='+condition+'&actionetap_condition_content='+condition_content+'&actionetap_date_fin='+date_fin+'&id_metacateg='+id_etap+'&actionetap_condition_label_yes='+condition_label_yes+'&actionetap_condition_label_no='+condition_label_no+'&actionetap_type_etape='+type_etape);
	//dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';
}

function addDocReturn(id_doc,id_etap) {
	label	= dims_getelem('input_label').value;
	text	= dims_getelem('input_content').value;
		labelen		= dims_getelem('input_label_en').value;
	texten	= dims_getelem('input_content_en').value;
	dims_xmlhttprequest_post('admin.php', 'action=save_etap_docreturn&id_etap='+id_etap+'&id_doc='+id_doc+'&input_content='+text+'&input_label='+label+'&input_content_en='+texten+'&input_label_en='+labelen);
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';
}

function moveEtape(id_etap, move) {
	dims_xmlhttprequest_post('admin.php', 'action=saveetap_position&id_etap='+id_etap+'&move='+move);
	dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
}

function makePayementEtap(id_etap) {
	dims_xmlhttprequest_post('admin.php', 'action=make_payement_etap&id_etap='+id_etap);
	dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
}

function unmakePayementEtap(id_etap) {
	dims_xmlhttprequest_post('admin.php', 'action=unmake_payement_etap&id_etap='+id_etap);
	dims_xmlhttprequest_todiv('admin.php', 'action=refresh_etap&subaction=<?php echo _DIMS_ACTION_ETAP; ?>&id_actionetap=0', '', 'block_content5');
}
...
function addFile() {
	...
	dims_xmlhttprequest_post('admin.php', 'action=save_eventfile&id_etap='+id_etap);
	document.location.href='admin.php?dims_mainmenu=events&submenu=<?php echo _DIMS_VIEW_EVENTS_DETAILS; ?>&action=add_evt&id=<?php echo $_SESSION['dims']['currentaction']; ?>';

}
</script>
<div class="lien_news_header">
	<a class="selected" onclick="javascript:change_menu(0);" href="javascript:void(0);"><?php echo $_SESSION['cste']['_EVENT_DETAILS']; ?></a>
		<a onclick="javascript:change_menu(1);" href="javascript:void(0);"><?php echo $_DIMS['cste']['_DIMS_LABEL_DESCRIPTION'];?></a>
		<?php
		if($type == dims_const::_PLANNING_ACTION_EVT && $typeaction != '_DIMS_PLANNING_FAIR_STEPS' && $enabledAdminEvent && $id > 0) {
		?>
		<a onclick="javascript:change_menu(3);" href="javascript:void(0);"><?php echo $_DIMS['cste']['_DIMS_EVT_CONTACT_REGISTER'];?></a>
		<?php
		}

		if($_SESSION['dims']['currentaction']>0 && $type == dims_const::_PLANNING_ACTION_EVT) {
				if($niveau==2 && $typeaction == '_DIMS_PLANNING_FAIR_STEPS') {
		?>
		<a onclick="javascript:change_menu(5);" href="javascript:void(0);"><?php echo $_DIMS['cste']['_DIMS_MILESTONE'];?></a>
		<?php
			}
			if($typeaction != '_DIMS_PLANNING_FAIR_STEPS' && $enabledAdminEvent) {
			?>
			<a onclick="javascript:change_menu(6);" href="javascript:void(0);"><?php echo $_SESSION['cste']['_EVENT_RESSOURCE_FRONT'];?></a>
			<?php
			}
		}
		?>
...
<div style="width:100%;">
	<div style="float:left;width:100%;">
		...
		<div class="onglettoolbar" style="margin-top:5px;margin-left:2px;">
			<div class="onglettoolbar_left">
				...
				<?php
					if(false) { //$typeaction != '_DIMS_PLANNING_FAIR' && $typeaction != '_DIMS_PLANNING_FAIR_STEPS'
				?>
				<div style="width: 120px;" id="block2" class="onglettoolbar_icon">
					<a onclick="javascript:change_menu(2);" href="javascript:void(0);">
						<div class="onglettoolbar_icon_image">
							<?php
							if($type == dims_const::_PLANNING_ACTION_EVT) $img_ent="mini_ent.png";
				...
				<?php
								require_once(DIMS_APP_PATH . "/modules/wce/class_wce_site.php");
								$wcesite = new wce_site($db);
								$urlarticle=$wcesite->getArticleByObject('events','events',$extraparams); // name of moduletype, name of label wce object
				$link=$urlarticle.'/index.php?id_event='.$action->fields['id']."&action=form_niv1";
	...
	<?php
	require_once(DIMS_APP_PATH . '/modules/events/public_events_modifier_action_detail.php');
...
/*
// bloc de tags
if ($action->fields['id']>0 && $typeaction != '_DIMS_PLANNING_FAIR_STEPS') {
	// on a une action enregsitrée
	$_SESSION['dims']['current_object']['id_record']=$action->fields['id'];
	$_SESSION['dims']['current_object']['id_module']=1;
	$_SESSION['dims']['current_object']['id_object']=dims_const::_SYSTEM_OBJECT_EVENT;
	echo $skin->open_widgetbloc($_DIMS['cste']['_DIMS_LABEL_TAGS'], 'width:100%;clear:both;', 'padding-bottom:1px;padding-left:10px;vertical-align:bottom;font-weight: bold;', '','26px', '26px', '-15px', '-7px', '', '', '');
...
<?php
	if (!empty($_SESSION['dims']['eventstep']) && $id>0) {

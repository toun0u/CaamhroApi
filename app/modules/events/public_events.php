...
switch($dims_op) {
				...
				<xs:element name="event">
				  <xs:complexType>
					<xs:sequence>
					  ...
					  <xs:element name="dateextended" type="xs:numeric"/>
	...
	default:
		...
		foreach($front as $id=>$worksp) {
			if ($worksp['code']==$workspace_code && $workspace_code!='') {
...
//echo 'ici'.$workspace_code.':'.$id_workspace;die();
		...
		if (!empty($events)) {
			$sql = 'SELECT		doc.id,
					...
					AND			doc.id_record in ('.implode(',',$events).')
					ORDER BY	id_record,id,version';

			$res = $db->query($sql, array(':idmodule' => $id_module, ':idobject' => $id_object) );
		...
		if (!empty($events)) {
			...
			$res = $db->query($sql, array(':idmodule' => $id_module, ':idobject' => $id_object_part) );
		...
		$sql = 'SELECT
				...
				WHERE
					a.type = :type
					and a.typeaction !="_DIMS_PLANNING_FAIR_STEPS"
				/*AND
					a.allow_fo = 1
				AND
					a.close = 0*/
		...
		if ($id_workspace>0) {
			...
			$param[':idworkspace'] = $id_workspace;
		...
		if($db->numrows($ressource)) {
			...
			while($result = $db->fetchrow($ressource)) {
				...
				if(!isset($tab_info['id'])) {
					...
					$tab_info['dateextended']	= $result['dateextended'];
					//$tab_info['banner']	= $result['banner_path'];

					if ($result['matchmaking_path']!="" && file_exists(realpath("./").str_replace("./","/",$result['matchmaking_path']))) {
						$webmatchpath=$dims->getProtocol().$dims->getHttpHost().substr($result['matchmaking_path'],1);
					...
					if($result['allow_fo'] == 1 && $tab_info['closed']=='false') {
						$link['link']=dims_urlencode($dims->getProtocol().$dims->getHttpHost().'/index.php?id_event='.$result['id_evt'].'&action=form_niv1',true);
					}
					else {
						$link['link'] = '';
					}
					...
					if ($result['banner_path']!="" && file_exists(realpath("./").str_replace("./","/",$result['banner_path']))) {
						...
						/* if ($result['id_evt']==10669) {
							dims_print_r($result);
							echo $pathtemp;
							die();
						}*/
						$webbannerpath=$dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'event_file/banner_'.$result['id_evt'].'.'.$extension;
							...
							if (isset($picture_extension[$pict['extension']])) {
							   /* $lk_booklet=array();
								...
								if (file_exists($path)) {
									...
									$lk_booklet['link_mini'] = $dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'doc-'.$pict['id_module']._DIMS_SEP.substr($pict['timestp_create'],0,8)._DIMS_SEP.$pict['id'].'_'.$pict['version'].'_gal.'.$pict['extension'];

									$lk_booklet['link'] = $dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'doc-'.$pict['id_module']._DIMS_SEP.substr($pict['timestp_create'],0,8)._DIMS_SEP.$pict['id'].'_'.$pict['version'].'_biggal.'.$pict['extension'];
								...
								*/
								...
								$lk_booklet['name'] = $pict['name'];
								$lk_booklet['description'] = $pict['description'];
								$lk_booklet['link'] = $dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'doc-'.$pict['id_module']._DIMS_SEP.substr($pict['timestp_create'],0,8)._DIMS_SEP.$pict['id'].'_'.$pict['version'].'.'.$pict['extension'];
								...
								if ($result['preview_path']!="" && file_exists(realpath("./").str_replace("./","/",$result['preview_path']))) {
									...
									$webpreviewpath=$dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'event_file/preview_'.$result['id_evt'].'.'.$extension;
						...
						foreach ($partners[$result['id_evt']] as $pict) {
							...
							if (file_exists($path)) {
								if (!file_exists($pathtemp)) {
									...
									$partner['link'] = $dims->getProtocol().$dims->getHttpHost().'/'._DIMS_WEBPATHDATA.'doc-'.$pict['id_module']._DIMS_SEP.substr($pict['timestp_create'],0,8)._DIMS_SEP.$pict['id'].'_'.$pict['version'].'_160.'.$pict['extension'];
								}
							}
				...
				foreach($tab_evt as $k => $event) {
					...
					foreach($event as $k => $elem) {
						...
						if (is_array($elem)) {
									...
									if (is_array($elemi)) {
										foreach($elemi as $kj =>$elemj) {
											...
											foreach($elemj as $kk => $elemk) {
													echo '<'.$kk.'>';
													//mb_detect_encoding($elemk, "UTF-8") == "UTF-8" ? : $elemk = utf8_encode($elemk);
													//$elemk = utf8_encode($elemk);
													echo $elemk;
													echo '</'.$kk.'>';
											}
											echo '</'.$kj.'>';
							...
							//mb_detect_encoding($elem, "UTF-8") == "UTF-8" ? : $elem = utf8_encode($elem);

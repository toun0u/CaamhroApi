...
$res_q = $db->query($sql, array(':id' => $_SESSION['sharefile']['current_share']));
...
if($value = $db->fetchrow($res_q)) {
	...
	if ((dims_isactionallowed(0) || dims_isadmin())) {
		...
		echo $skin->open_simplebloc("Modifier la question",'100%','','',false);
			...
			$res=$db->query("SELECT * FROM dims_mod_faq_rubriques WHERE id_module= :idmodule ", array(':idmodule' => $_SESSION['dims']['moduleid']) );
			...
			$answer = $db->query($select, array(':idmodule' => $_SESSION['dims']['modules'][$_SESSION['dims']['moduleid']]['id_module_type']) );
			$tabroles = array();
			...
			if (sizeof($tabroles)>0) {
				...
				$select =	"
							...
							AND			id_role in (".implode($tabroles,",").")";
	...
	$sql_r = "SELECT r.*, r.id as id_r, u.lastname,u.firstname, u.id as id_user FROM dims_mod_faq_reponse r INNER JOIN dims_user u ON u.id = r.id_user WHERE r.id_question = :idquestion ";
	$res_r = $db->query($sql_r, array(':idquestion' => $id_share) );
	$tab_r =array();

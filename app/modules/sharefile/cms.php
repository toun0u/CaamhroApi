				...
				if (dims_isadmin() || $share->isOwner($_SESSION['dims']['userid'])) {
					...
					if ($shareuser->fields['id_user']>0) {
						...
						$res=$db->query("SELECT DISTINCT u.id, u.email, u.firstname,u. lastname,fu.code
										FROM dims_user as u
										LEFT JOIN dims_mod_sharefile_user as fu on fu.id_user=u.id and fu.id_share= :idshare
										WHERE u.id = :id ",array(
									':idshare' => $share->fields['id'],
									':id' => $shareuser->fields['id_user']
								));
						if ($db->numrows($res)>0) {
						...
						$res=$db->query("SELECT DISTINCT c.id, c.email, c.firstname,c.lastname,fu.code
										...
										LEFT JOIN dims_mod_sharefile_user as fu on fu.id_contact=c.id
										WHERE fu.id_share= :idshare
										AND c.id_user= :iduser
										AND c.id = :id ", array(
										':idshare' => $share->fields['id'],
										':iduser' => $_SESSION['dims']['userid'],
										':id' => $shareuser->fields['id_contact']
									));
						if ($db->numrows($res)>0) {
							while ($u=$db->fetchrow($res)) {
								$emailto=$u['email'];

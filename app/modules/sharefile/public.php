...
$url->setProtocol(dims::getInstance()->getProtocol());
if(dims::getInstance()->getProtocol()=='https://') $url->setPort('443');
if(!empty($articleid)) $url->addParam('article_id', $articleid);

$view->assign('urlbase', $url);

if(!empty($obj['module_id'])) $moduleid=$obj['module_id'];
else $moduleid=$_SESSION['dims']['moduleid'];

// recherche des params par defaut
$sharefile_param = new sharefile_param();
$sharefile_param->verifParam($moduleid);

$workspaces = dims_viewworkspaces($moduleid);
		...
		switch($action) {
			...
			case 'main_menu':
				...
				$sqlshare= "SELECT s.*, u.lastname,u.firstname,count(f.id_doc) as cptefile
						...
						ON			f.id_share=s.id
						WHERE		s.deleted=0 and s.id_user= :iduser and s.id_module= :idmodule
						GROUP BY	s.id order by timestp_create desc";
				$res_s = $db->query($sqlshare, array(':iduser' => $_SESSION['dims']['userid'], ':idmodule' => $moduleid) );
				...
				break;
			case 'admin_params':
				if(!dims::getInstance()->isAdmin()) dims_redirect($url->addParams(array('op' => 'sharefile', 'action' => 'main_menu')));
				// recherche des params par defaut
				$sharefile_param = new sharefile_param();
				$sharefile_param->verifParam($_SESSION['dims']['moduleid']);

				require_once(DIMS_APP_PATH . "/modules/sharefile/public_param.php");
				break;
			case "sharefile_saveparam":
				if(isset($_SESSION['dims']['connected']) && $_SESSION['dims']['connected'] && dims::getInstance()->isAdmin()) {
					// recherche des params par defaut
					$sharefile_param = new sharefile_param();
					$sharefile_param->verifParam($_SESSION['dims']['moduleid']);
					$sharefile_param->setvalues($_POST, "share_param_");

					$sharefile_param->save();

					dims_redirect($url->addParams(array('op' => 'sharefile', 'action' => 'admin_params')));
				}
				break;
		}
		break;
		...
		switch($action) {
			...
			case 'second_step':
				...
				if($share->hasEntity()) {
					...
					$sql = 'SELECT * FROM '.contact::TABLE_NAME.' WHERE id_globalobject IN ('.implode(',', array_keys($result['distribution']['contacts'])).')';

					$res = $db->query($sql);

					while($data = $db->fetchrow($res)) {
						if(!$share->isContactLinked($data['id'])) {
							$contactFields[] = $data;
						}
					}

					$view->assign('entity', $entity->fields);
					$view->assign('entityContacts', $contactFields);
				}
...
*/
				...
				$sqlContact =
					...
					WHERE		(contact.id_user = :iduser
					OR			contact.private = 0)
					AND			(contact.lastname LIKE :nomsearch
					OR			contact.firstname LIKE :nomsearch
					OR			contact.email LIKE :nomsearch )';

				$res = $db->query($sqlContact, array(':iduser' => $_SESSION['dims']['userid'], ':nomsearch' => "%$nomsearch%") );
				...
				die();
				break;
			case 'refresh_entity_contact':
				if(!$_SESSION['dims']['connected']) dims_redirect($url->addParams(array('op' => 'sharefile', 'action' => 'connection')));

				$share = unserialize($_SESSION['share']['object']);

				$searchResult = array( 'contacts' => array() );
			...
			case 'save_share':
				...
				if($share->hasEntity()) {
					...
					$curModule->open($moduleid);

					$moduleType = new module_type();
					$moduleType->open($curModule->fields['id_module_type']);

					$extraparams = '';
					$articlesIds = array();
					$baseUrl = $wce_site->getArticleByObject($moduleType->fields['label'], 'Sharefile', $extraparams, -1, $articlesIds);
					...
					$url = new dims_urlBuilder('index.php', '/', $domain['domain'], $port, $protocol);
					$url->addParam('articleid', current($articlesIds));
				}
				...
				foreach($share->getUsers() as $u) {
					...
					$link='<a href="'.dims_urlencode($url->addParams(array('op' => 'share', 'action' => 'view', 'id_share' => $share->fields['id'], 'id_user' => $u['id'])).$extraparams).'">Acc&eacute;der au partage</a>';
				...
				foreach($share->getContacts() as $u) {
					...
					// construction du lien
					$link='<a href="'.dims_urlencode($url->addParams(array('op' => 'share', 'action' => 'view', 'id_share' => $share->fields['id'], 'id_contact' => $u['id'])).$extraparams).'">Acc&eacute;der au partage</a>';
				...
				if (dims_isadmin() || $share->isOwner($_SESSION['dims']['userid'])) {
					...
					if ($shareuser->fields['id_user']>0) {
						...
						$res=$db->query("SELECT DISTINCT u.id, u.email, u.firstname,u. lastname,fu.code
										...
										WHERE u.id = :iduser ",
										array(':idshare' => $share->fields['id'], ':iduser' => $shareuser->fields['id_user']));
						if ($db->numrows($res)>0) {
						...
						$res=$db->query("SELECT DISTINCT c.id, c.email, c.firstname,c.lastname,fu.code
										...
										AND c.id = :idcontact ",
										array(':idshare' => $share->fields['id'], ':iduser' => $_SESSION['dims']['userid'], ':idcontact' => $shareuser->fields['id_contact']) );
						if ($db->numrows($res)>0) {
							while ($u=$db->fetchrow($res)) {
								$emailto=$u['email'];
				...
				if ($_SESSION['currentshare']['id_share']==0) {
					...
					elseif ($share->fields['timestp_finished']>$maxtoday && !isset($_SESSION['dims']['userid']) && !$share->isOwner($_SESSION['dims']['userid'])) {
						...
						if (!empty($_GET['docfile_md5id'])) {
							$docfilemd5id = dims_load_securvalue('docfile_md5id', dims_const::_DIMS_CHAR_INPUT, true, true, true);
							$resultat=$share->verifAccessFile($docfilemd5id);
							if ($resultat) {
								// on peut downloader le fichier
								$res=$db->query("SELECT id FROM dims_mod_doc_file WHERE md5id = :md5 ", array(':md5' => $docfilemd5id) );
			...
			case "sharefile_maxdate":
				require_once(DIMS_APP_PATH . "/modules/sharefile/sharefile_maxdate.php");
				break;
			case "sharefile_maxdownload":
				require_once(DIMS_APP_PATH . "/modules/sharefile/sharefile_maxdownload.php");
				break;
			case "sharefile_deleted":
				require_once(DIMS_APP_PATH . "/modules/sharefile/sharefile_deleted.php");
				break;
